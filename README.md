...
Limited live fetch dry-run is scoped only; no network code has been added yet.（見 [docs/limited-live-fetch-dry-run-pr-scope.md](docs/limited-live-fetch-dry-run-pr-scope.md)；`npm run test:limited-live-fetch-dry-run-pr-scope` 確認 runtime 檔案仍無 fetch / axios / process.env / @supabase / /api/portfolio / PRODUCTION_READY）。
