...
- `use-cases/runtime-pilot/`：Runtime Pilot Readiness Checklist read-model type contract 與 pure spec builder（types + 靜態安全 constants，無 runtime；18 個 readiness gates / severity / status / go-no-go decision / audit / rollback / kill switch shape；預設 decision = NO_GO）。
