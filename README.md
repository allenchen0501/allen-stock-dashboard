...
Every completed version must include a Project Handoff Summary in the final report. The handoff must summarize production commit, safety-chain status, completed scope, permanent red lines, current phase restrictions, standalone validators, current live-fetch state, UI language status, future allowed direction, and next recommended work.（**every future completed version must include Project Handoff Summary**；見 [docs/project-handoff-summary.md](docs/project-handoff-summary.md) 與 [docs/project-red-lines-and-roadmap.md](docs/project-red-lines-and-roadmap.md)，`npm run test:project-handoff-summary` 檢查。）
...
Cross-Module Consistency & Candidate Ranking Governance adds a fixture-only governance layer that detects conflicts between Allen Score, 17-Line Power Score, Technical + Risk Reward, 扣三低, and Position Strategy. It is observation-only, does not fetch real data, does not switch production data, and does not generate buy/sell commands.（跨模組一致性檢查與候選排序治理：偵測同一檔股票在各模組間的方向矛盾，輸出 conflictLevel（none/warning/critical）、hardGateStatus、rankingEligible 與繁中 finalObservationLabelZh（如「訊號分歧，暫不升級」「禁碰優先」「趨勢強但資料不足」「扣三低通過但趨勢未確認」「趨勢與拉回條件一致」）；**Position Strategy 禁碰凌駕所有高分訊號**；候選排序分 Hard Gates + Weighted Observation Score 兩層；風報比連續分級（continuous range）且**風報比不是勝率**，在 setupWinRate / backtest 完成前不得作為期望值排序；Technical Score Collinearity Guard 僅建 spec、不重算 Allen Score 100；目前門檻皆未回測校準、不得宣稱統計勝率。整合進 War Room read model 與 UI「跨模組一致性」區塊（標示非買賣建議／非進場訊號／非自動下單）。fixture-only、standalone validator、不納入 safety-chain、approved live-fetch symbols 仍只有 3019。見 [docs/cross-module-consistency-ranking-governance.md](docs/cross-module-consistency-ranking-governance.md)；`npm run test:cross-module-consistency-governance`。此為 governance/consistency layer，不取代 Allen Score 100、Technical + Risk Reward、Position Strategy、扣三低、Allen 17-Line Power Score。）（真實行情驗證、多檔 watchlist、技術掃描器屬**未來允許方向**，需 owner approval + 分階段驗證 + manual sign-off，與自動交易／自動下單是兩回事。）「紅線」用語分為 **Permanent Red Lines / 永久紅線**（不可跨越：不替使用者下單、不自動交易、不自動執行買賣、未經 owner approval 不擴大正式 live fetch 股票池／不切正式資料／不接 broker API／不切 production data switch）與 **Current Phase Restrictions / 目前階段性限制**（暫時、未來可經核准解除）。

3019 Approved Live Quote Manual-Refresh MVP adds a read-only, manual-refresh live quote path for the single owner-approved symbol 3019 through the approved TWSE_TPEX channel tse_3019.tw. It does not auto-fetch on page load, does not switch /api/portfolio, does not connect Supabase, does not write data, and does not generate buy/sell commands or auto orders.（3019 核准真實報價手動刷新 MVP：新增唯讀真實報價路徑 `app/api/war-room/approved-live-quote`，只限 3019（亞光）、只限核准頻道 tse_3019.tw，沿用既有 approved provider `twse-tpex-verification-provider`（GET only、timeout 3000ms、maxRetries 0、field allowlist、任何失敗 fallback）。預設 War Room page load **不打真實行情**，只有使用者按下「手動刷新 3019」才取得一次唯讀報價；`price` / `sourceTimestamp` 不可取得時為 `null`，**不假造價格**；`dataStatus`（live_verified／fallback／timeout／source_error／not_available）皆有繁中顯示；非 3019／非 manual 一律 reject。非 production data switch、非 `/api/portfolio` switch、非 Supabase runtime、非交易訊號、非買賣建議、非下單、非自動交易；approved live-fetch symbols 仍只有 3019。整合進 War Room UI「3019 核准真實報價」區塊（手動刷新按鈕、loading／success／fallback／error、非買賣建議／非進場訊號／非自動下單）。standalone validator、不納入 safety-chain。見 [docs/approved-live-quote-3019-manual-refresh-mvp.md](docs/approved-live-quote-3019-manual-refresh-mvp.md)；`npm run test:approved-live-quote-3019-mvp`。可選擇性 `npm run smoke:limited-live-fetch:3019`（manual smoke，不納入 safety-chain）。future core 5 read-only expansion（4966／5347／4979／2455）require separate owner approval。）

3019 Approved Live Quote Production Smoke verifies the production manual-refresh path for the single owner-approved symbol 3019. It documents approved manual request behavior, non-approved symbol rejection, manual-mode enforcement, default page-load no-fetch, and safe fallback/error UI. It is read-only, manual-only, not in safety-chain, and does not expand live-fetch symbols.（3019 核准真實報價線上手動刷新驗證：記錄 production commit 36a7220 / deployment dpl_FWMFvQd4poKngGWHBsiYTecAXdMx 的 manual smoke evidence — approved 手動請求（symbol=3019&mode=manual）、非核准代號拒絕（symbol=4966 不 fetch）、manual 模式強制（mode=auto 拒絕）、預設 page load 不 fetch、fallback／timeout／source_error 安全 UI 不崩潰。real-network smoke（`npm run smoke:limited-live-fetch:3019`）結果 LIVE_FETCH_OK、price 為真實來源值不假造；若 source 不可用只回報 fallback／timeout／source_error／not_available、price=null。read-only、manual-only、不納入 safety-chain、不擴大 live-fetch 股票池、approved live-fetch symbols 仍只有 3019。見 [docs/approved-live-quote-3019-production-smoke-report.md](docs/approved-live-quote-3019-production-smoke-report.md)；`npm run test:approved-live-quote-3019-production-smoke`。此為 verification／evidence step，非 core 5 擴充、非正式行情全切、非 `/api/portfolio` switch、非交易訊號。）

3019 Production Endpoint Case Evidence records the live production endpoint behavior for the approved 3019 manual request and the rejection paths for non-approved symbols, missing manual mode, and auto mode. It is evidence-only, read-only, not in safety-chain, and does not expand approved live-fetch symbols.（3019 線上 Endpoint 實測案例證據補齊：直接對 production URL `https://allen-stock-dashboard.vercel.app/api/war-room/approved-live-quote` 實跑 4 個 case 並記錄實際回應 — Case A（3019&manual）→ HTTP 200、live_verified、price 140、sourceTimestamp 2026-07-01T06:30:00.000Z、requestPerformed=true；Case B（4966&manual）→ not_available、requestPerformed=false、price=null、reasonZh 非核准代號；Case D（3019&auto）→ not_available、requestPerformed=false、price=null、reasonZh 僅允許手動刷新。本版並將 route 收緊為 `mode` 必須明確為 `manual`（缺少 mode 不再預設 manual，改為拒絕），Case C（3019 缺 mode）在舊部署仍會 fetch 已如實記錄、收緊後將於下一次 deployment 生效驗證。無假造價格；approved live-fetch symbols 仍只有 3019；evidence-only、read-only、不納入 safety-chain。見 [docs/approved-live-quote-3019-production-smoke-report.md](docs/approved-live-quote-3019-production-smoke-report.md)；`npm run test:approved-live-quote-3019-production-endpoint-cases`。此非 core 5 擴充、非正式行情全切、非 `/api/portfolio` switch、非 Supabase runtime、非交易訊號、非下單、非自動交易。）

3019 Post-Deployment Explicit Manual Mode Verification records the production behavior after ec31db1 tightened the endpoint to require explicit mode=manual. It verifies the approved 3019 manual path and confirms non-approved symbols, missing mode, and auto mode are rejected without fetch.（3019 明確手動模式部署後驗證：針對已部署的 ec31db1 重跑 production endpoint case — Case A（3019&manual）→ live_verified、price 140；Case B（4966&manual）→ not_available、requestPerformed=false；Case C（3019 缺 mode）→ **收緊後已拒絕** not_available、requestPerformed=false、reasonZh 手動模式必須（取代舊部署 dea7f4e 缺 mode 仍 fetch 的 stale evidence）；Case D（3019&auto）→ not_available、requestPerformed=false。4 case 皆 HTTP 200、無假造價格；approved live-fetch symbols 仍只有 3019；evidence-only、read-only、不納入 safety-chain。見 [docs/approved-live-quote-3019-production-smoke-report.md](docs/approved-live-quote-3019-production-smoke-report.md)；`npm run test:approved-live-quote-3019-post-deployment-explicit-manual`。此非 core 5 擴充、非正式行情全切、非 `/api/portfolio` switch、非 Supabase runtime、非交易訊號、非下單、非自動交易。）

Core 5 Read-Only Expansion Approval Spec defines the owner-approval workflow and per-symbol evidence gates each of the core 5 Taiwan symbols must clear before it may ever enter the runtime approvedLiveFetchSymbols. It is a fixture-only governance spec: only 3019 is approved; 4966 / 5347 / 4979 / 2455 stay pending owner approval and are NOT added to approvedLiveFetchSymbols. It does not expand live fetch, add a provider/channel to the runtime approved set, switch production data, or touch /api/portfolio.（核心 5 檔唯讀擴充核准規格：核心 5 檔為 3019 亞光、4966 譜瑞、5347 世界、4979 華星光、2455 全新。目前只有 3019 已核准並完成 production endpoint verification；4966／5347／4979／2455 皆 `pending_owner_approval`，**不得**加入 approvedLiveFetchSymbols。每一檔進入 approved set 前必須完成：owner approval → channel verification → per-symbol standalone validator → manual-refresh-only + read-only → non-approved / missing-mode / auto-mode rejection cases → manual smoke evidence → production endpoint evidence（缺一不可）。候選 symbol 與 proposed channel 皆為 spec-only，**未進入 runtime**（provider／route／MVP contract／dashboard 皆不含 4966／5347／4979／2455）；approvedLiveFetchSymbols 仍只有 3019。fixture-only、deterministic、standalone validator、不納入 safety-chain。見 [docs/core-5-read-only-expansion-approval-spec.md](docs/core-5-read-only-expansion-approval-spec.md)；`npm run test:core-5-expansion-approval-spec`。此為 approval governance spec，不是開第二檔、不是 live-fetch expansion、不是正式行情全切、不是 `/api/portfolio` switch、不是 Supabase runtime、不是交易訊號、不是買賣建議、不是下單、不是自動交易；不因 3019 成功就直接擴大到 core 5。）

4966 Owner Approval Packet & Channel Verification Prep prepares 4966 (譜瑞) as the next read-only manual-refresh candidate. It is a fixture-only owner-approval packet: 4966 stays candidate_only / pending_owner_approval / not approved, approvedChannel=null (proposed channel unverified), not in approvedLiveFetchSymbols, with no live-fetch endpoint, no War Room button, no smoke executed, and no production endpoint called. It requires explicit Allen owner approval before the next version.（4966 譜瑞核准包與通道驗證準備：為 4966 準備 owner approval packet、channel verification prep、per-symbol required evidence checklist、future manual smoke plan（不執行）、future production endpoint cases（不執行）、owner approval record schema 與 owner 核准所需文字。本版不核准 4966、不加入 approvedLiveFetchSymbols、不新增 4966 endpoint、不新增 War Room manual refresh button、不執行 smoke、不打 production endpoint；proposedChannel `tse_4966.tw` 標記 proposed／unverified／not approved，approvedChannel=null。候選 symbol／proposed channel 皆 spec-only，未進入 runtime（provider／route／MVP contract／dashboard 皆不含 4966）；approvedLiveFetchSymbols 仍只有 3019。fixture-only、deterministic、standalone validator、不納入 safety-chain。後續仍需 Allen owner 明確核准，才能進入下一版「4966 Read-Only Manual-Refresh MVP」。見 [docs/4966-owner-approval-packet.md](docs/4966-owner-approval-packet.md)、[docs/4966-channel-verification-prep.md](docs/4966-channel-verification-prep.md)；`npm run test:4966-owner-approval-packet`。此非開第二檔、非 live-fetch expansion、非正式行情全切、非 `/api/portfolio` switch、非 Supabase runtime、非交易訊號、非買賣建議、非下單、非自動交易；不因 3019 成功就直接擴大到 4966。）

4966 Owner Approval Decision Gate defines exactly how Allen owner must explicitly approve 4966 (譜瑞) before the next version, including the approval decision states, state-transition rules, the required owner phrase, the safety-boundary elements that phrase must contain, and the preconditions/remaining requirements after approval. It is fixture-only and approves nothing: 4966 stays pending_owner_approval, approvalDate=null, approvedChannel=null, not in approvedLiveFetchSymbols, and the approval text is a template only.（4966 譜瑞 Owner 核准決策閘門：定義 pending_owner_approval／approved／rejected 狀態與轉換規則、owner 核准句、核准句必含的安全邊界元素（read-only／manual-refresh-only／approved provider only／不切 portfolio API／不接 Supabase／不接 broker／不下單／不自動交易）、以及核准後仍需 channel verification + per-symbol validator + manual smoke + production endpoint evidence。本版**不核准 4966**：currentApprovalStatus 維持 pending_owner_approval、ownerApprovalReceived=false、approvalDate=null、approvedChannel=null、nextVersionUnlocked=false；approval text template 標記 templateIsApproval=false（列出模板不等於已核准）。4966 未進 approvedLiveFetchSymbols、無 endpoint／按鈕、未執行 smoke／production endpoint；候選未進 runtime（provider／route／MVP contract／dashboard 皆不含 4966）；approvedLiveFetchSymbols 仍只有 3019。fixture-only、deterministic、standalone validator、不納入 safety-chain。見 [docs/4966-owner-approval-decision-gate.md](docs/4966-owner-approval-decision-gate.md)；`npm run test:4966-owner-approval-decision-gate`。此非正式核准、非 live-fetch expansion、非正式行情全切、非 `/api/portfolio` switch、非 Supabase runtime、非交易訊號、非買賣建議、非下單、非自動交易。）

## UI 語言規則（Traditional Chinese UI）

所有**使用者可見的前台 UI 一律使用繁體中文**：頁面標題、區塊／卡片標題、按鈕、標籤、狀態、警示訊息、描述、儀表板文字、`/system/safety` 安全頁顯示文字、分析結論皆不得只出現英文。程式碼、檔名、變數、type、script、contract、validator、enum、JSON key、commit message 仍使用英文。UI 可保留必要技術縮寫（例如 API、CI、JSON、Supabase、TWSE、TPEx、mock、fixture、NO_GO、READY_FOR_CI_GUARD 等技術狀態碼／contract key），但旁邊需有繁中說明。布林狀態以 `zhBool()` helper 轉成繁中（是／否、允許／禁止、已完成／未完成…），不直接顯示 `true / false`；底層 contract key 不更動。`app/system/safety/page.tsx` 與其引用的 monitoring components（runtime-pilot-readiness／monitoring、first-authorized-source-dry-run-monitoring、shadow-runner-dry-run-monitoring、war-room/shadow-quote-comparison-card）已套用此規則；部分 safety-chain validator 仍需特定英文技術字串（例如卡片標題、`=false` 旗標、警示語），這些以「繁中（English 技術名）」並列保留，不破壞 safety-chain。System safety UI language validation verifies that user-visible system safety UI remains Traditional Chinese while allowing necessary technical identifiers. It remains standalone and does not change safety-chain composition.（`npm run test:system-safety-ui-language` 靜態檢查安全頁與 5 個前台元件：page heading／必要繁中關鍵詞、所有 `<h2>` 含中文且無純英文標題、monitoring 標題繁中為主（英文技術名僅在括號內）、card 繁中警示語、page + card 以 `zhBool()` 取代 `{String(…)}`、允許技術 token 但須有繁中說明；safety-chain 仍 22 checks，本 guard 與 smoke 皆不納入 `test:safety-chain`。見 [docs/system-safety-ui-language-guard.md](docs/system-safety-ui-language-guard.md)。）Monitoring component boolean UI readouts are localized to Traditional Chinese while numeric readouts remain numeric and technical status codes retain Chinese explanations.（4 個 monitoring 元件內部的 `{String(布林)}` 已逐欄位分類：布林改 `zhBool()`／`zhDisplay()`（是／否、允許／禁止…），數字保留 `String(number)`，狀態碼／id／時間戳保留技術值並加繁中說明；guard 新增 `05b_monitoring_no_raw_boolean` 檢查不再殘留 `String(<boolean>)` 或字面 true／false。）


目前進度為 Phase 2 — Implementation PR Scope & Locked Interface Contract（interface-only，未連線）：先定義未來 staging read-only real quote dry-run 會用到的型別 / interface / disabled provider / shadow comparison，但**不連線、不 fetch、不讀 env value**。新增 `use-cases/war-room/phase-2-locked-real-quote-contract.ts`（RealDataMode / RealQuoteProviderId / RealQuoteProviderStatus / ReadonlyQuoteSnapshot / QuoteProvider / ShadowQuoteComparison / Phase2LockedImplementationContract）、`use-cases/war-room/phase-2-disabled-real-quote-provider.ts`（DisabledRealQuoteProvider，getQuote 不 fetch）、`use-cases/war-room/build-phase-2-locked-implementation-contract.ts`、`docs/phase-2-implementation-pr-scope.md`、`scripts/validate-phase-2-locked-implementation.ts`。contractVersion = PHASE_2_LOCKED_INTERFACE、mode = INTERFACE_ONLY_NOT_CONNECTED、decision = NO_GO、defaultRealDataMode = fixture、shadowModeAllowed = false、realReadonlyAllowed = false，所有 connection 旗標 false。`npm run test:phase-2-locked-implementation`。**Phase 2 Safety Chain Integration：已把 `test:phase-2-locked-implementation` 納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 13 增加到 14；Phase 2 check critical=true / passed=true / decision NO_GO / mode INTERFACE_ONLY_NOT_CONNECTED，GitHub Actions Safety Chain workflow 自動涵蓋。** **Phase 2b Safety Chain Integration：再把 `test:phase-2b-shadow-comparison-ui-shell` 納入 `npm run test:safety-chain` 與 guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 14 增加到 15；Phase 2b check critical=true / passed=true / decision NO_GO / mode INTERFACE_ONLY_NOT_CONNECTED / real quote candidate status DISABLED / realDataConnected·fetchPerformed·envReadPerformed·supabaseConnected·productionReady 全 false。**no Supabase connection、no env read、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、no broker API、not PRODUCTION_READY。

Limited live fetch dry-run is scoped only; no network code has been added yet.（見 [docs/limited-live-fetch-dry-run-pr-scope.md](docs/limited-live-fetch-dry-run-pr-scope.md)；`npm run test:limited-live-fetch-dry-run-pr-scope` 確認 runtime 檔案仍無 fetch / axios / process.env / @supabase / /api/portfolio / PRODUCTION_READY）。**Limited Live Fetch Scope Safety Chain Integration：已把 `test:limited-live-fetch-dry-run-pr-scope` 納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 16 增加到 17；scope check critical=true / passed=true / decision NO_GO / mode SCOPE_ONLY_NO_NETWORK_CODE / networkCodeAdded·liveFetchAllowed·envReadPerformed·supabaseConnected·apiRouteCreated·portfolioApiSwitched·productionReady 全 false，正式 owner approval 前不得被偷加 network code。**

Limited live fetch dry-run implementation exists for approved TWSE/TPEx provider only, first symbol 3019 only, shadow-only, default fixture, not production ready.（唯一允許受限 fetch 的檔案是 `services/market-data/twse-tpex-verification-provider.ts`；channel tse_3019.tw、GET only、timeout=3000ms、maxRetries=0、field allowlist、任何失敗 fallback disabled scaffold candidate；預設 `dryRunLiveFetch=false`，app 不會預設 live fetch；operationalUseAllowed=false、no /api/portfolio switch、no API route、no Supabase、no process.env、no DB write、no broker API、no buy/sell command、no auto order、not production ready。見 [docs/limited-live-fetch-dry-run-implementation.md](docs/limited-live-fetch-dry-run-implementation.md)。`npm run test:limited-live-fetch-dry-run-implementation` 靜態驗證；`npm run smoke:limited-live-fetch:3019` 為 manual smoke script only，**不在** safety chain，失敗只顯示 fallback、不影響 build。approved provider 的 `fetch(` 已在 `test:limited-live-fetch-dry-run-pr-scope` 與 `test:staging-shadow-runtime-scaffold` 中被豁免，其餘 forbidden token 仍照舊強制，其他檔案仍不得 fetch。implementation validator 待 owner review 後才會單獨併入 safety chain。）**Limited Live Fetch Implementation Safety Chain Integration：已把 `test:limited-live-fetch-dry-run-implementation` 納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 17 增加到 18；implementation check critical=true / passed=true / decision LIVE_FETCH_DRY_RUN_NON_OPERATIONAL / mode LIMITED_LIVE_FETCH_DRY_RUN_SHADOW_ONLY / approvedProviderOnly=true / approvedProvider=TWSE_TPEX / symbol=3019 / channel=tse_3019.tw / timeoutMs=3000 / maxRetries=0 / GET only / fallbackDisabledScaffoldCandidate=true / defaultRealDataMode=fixture / shadowOnly=true / operationalUseAllowed·portfolioApiSwitchAllowed·productionReady·brokerApiAllowed·buySellCommandGenerated·autoOrderRequested 全 false，GitHub Actions Safety Chain workflow 自動涵蓋。新增 `use-cases/war-room/build-limited-live-fetch-implementation-contract.ts`（deterministic / pure，不執行 live fetch）。The manual smoke script `smoke:limited-live-fetch:3019` is manual only and is NOT part of the safety chain（guard validator 明確檢查 `test:safety-chain` 與 CHAIN_SPECS 都不含此 smoke script）。**

Limited live fetch 3019 observation round 1 is recorded as manual smoke evidence only; it is not a production data switch and does not feed trading decisions.（見 [docs/limited-live-fetch-3019-observation-round-1.md](docs/limited-live-fetch-3019-observation-round-1.md)；`npm run test:limited-live-fetch-3019-observation-round-1` 靜態驗證 observation doc 與 approved scope，不要求固定 price；此 validator 與 smoke script 皆 **不**納入 `npm run test:safety-chain`。）

Limited live fetch 3019 observation round 2 is recorded as manual smoke evidence only; it is not a production data switch and does not feed trading decisions.（見 [docs/limited-live-fetch-3019-observation-round-2.md](docs/limited-live-fetch-3019-observation-round-2.md)；`npm run test:limited-live-fetch-3019-observation-round-2` 靜態驗證 round 2 doc、approved scope 與 Round 1 比對，不要求固定 price；此 validator 與 smoke script 皆 **不**納入 `npm run test:safety-chain`。）

Limited live fetch now supports optional deterministic `receivedAt` clock injection for tests; runtime default remains unchanged and it does not expand provider/symbol scope.（`PublicReadonlyQuoteCandidateOptions.now?: () => Date`；不傳則用 real clock，live candidate 與 disabled scaffold fallback candidate 皆可注入同一 clock 做 golden snapshot test。見 [docs/limited-live-fetch-deterministic-clock.md](docs/limited-live-fetch-deterministic-clock.md)；`npm run test:limited-live-fetch-deterministic-clock` 靜態驗證；此 validator **不**納入 `npm run test:safety-chain`，safety chain CI guard 仍 18 checks。）

Limited live fetch golden snapshot validation uses fixed TWSE mock data and deterministic clock injection; it does not perform live fetch and remains outside the safety chain initially.（離線驅動已 export 的 pure parser `parseLimitedLiveFetchResponse`，fixed now=2026-06-30T00:00:00.000Z、fixed mock（c=3019/z=142.5…），驗證 success candidate 與 wrong-symbol fallback candidate 的 golden snapshot；不打網路、不跑 smoke。見 [docs/limited-live-fetch-golden-snapshot.md](docs/limited-live-fetch-golden-snapshot.md)；`npm run test:limited-live-fetch-golden-snapshot`；此 validator 與 smoke script 皆 **不**納入 `npm run test:safety-chain`，safety chain CI guard 仍 18 checks。）

Limited live fetch golden snapshot validation now includes an offline fallback matrix for malformed TWSE-like responses; it remains outside the safety chain and performs no live fetch.（12 個 hard-fallback case：empty/missing msgArray、wrong symbol、wrong channel、missing/invalid/non-numeric price、missing/invalid tlong、malformed root/entry，皆回 disabled scaffold candidate；另 2 個 optional-field case（missing/non-numeric volume）保持 non-operational。見 [docs/limited-live-fetch-golden-fallback-matrix.md](docs/limited-live-fetch-golden-fallback-matrix.md)；沿用 `npm run test:limited-live-fetch-golden-snapshot`，仍 **不**納入 safety chain、不打網路、不跑 smoke。）

Golden snapshot validation for limited live fetch is now part of the safety chain as an offline deterministic parser/fallback matrix guard. It performs no live fetch and does not include manual smoke.（**Golden Snapshot Safety Chain Integration**：`test:limited-live-fetch-golden-snapshot` 已納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 18 增加到 **19**；golden check critical=true / passed=true / decision OFFLINE_DETERMINISTIC_SNAPSHOT_OK / mode OFFLINE_DETERMINISTIC_PARSER_SNAPSHOT / offline=true / deterministic=true / parserSnapshot=true / liveFetchPerformed=false / smokeInvoked=false / smokeManualOnly=true / productionDataSwitchAllowed=false / operationalUseAllowed·productionReady false / fallbackMatrixCaseCount≥10。manual smoke script 仍 **不**納入 safety chain；observation validators 與 deterministic clock validator 維持 standalone，本版不新增進 safety-chain。）

Limited live fetch mock fetch boundary validation verifies the approved TWSE channel and safe fallback behavior with mocked fetch only; it performs no real network request and remains outside the safety chain initially.（monkey-patch `globalThis.fetch` 後驅動真正的 `getTwseTpexLimitedLiveFetchCandidate`，fixed now=2026-06-30T00:00:00.000Z：success case 斷言 fetch 只呼叫一次、URL 僅含 approved channel `tse_3019.tw`、method GET、無 Yahoo/Supabase/broker；unsupported symbol 不 fetch、fetch error / malformed response 皆 safe fallback；測試後還原 fetch、`real_network_used=false`。見 [docs/limited-live-fetch-mock-fetch-boundary.md](docs/limited-live-fetch-mock-fetch-boundary.md)；`npm run test:limited-live-fetch-mock-fetch-boundary`；此 validator 已於下一版納入 safety chain（見下）。）

Mock fetch boundary validation for limited live fetch is now part of the safety chain as an offline deterministic request-boundary guard. It performs no real network request and does not include manual smoke.（**Mock Fetch Boundary Safety Chain Integration**：`test:limited-live-fetch-mock-fetch-boundary` 已納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 19 增加到 **20**；mock fetch check critical=true / passed=true / decision OFFLINE_DETERMINISTIC_BOUNDARY_OK / mode OFFLINE_DETERMINISTIC_REQUEST_BOUNDARY / offline=true / deterministic=true / mockFetchOnly=true / realNetworkUsed=false / fetchMockRestored=true / liveFetchPerformed=false / smokeInvoked=false / approvedSymbol=3019 / approvedChannel=tse_3019.tw / fetchCalledOnceForSuccessCase=true / unsupportedSymbolSafeFallback=true / fetchErrorSafeFallback=true / malformedResponseSafeFallback=true / productionDataSwitchAllowed·operationalUseAllowed·productionReady false。manual smoke script 仍 **不**納入 safety chain；observation validators 與 deterministic clock validator 維持 standalone，本版不新增進 safety-chain。）

Limited live fetch default no-fetch boundary validation verifies that the default runtime path performs no fetch unless dryRunLiveFetch is explicitly enabled. It remains standalone and performs no real network request.（spy `globalThis.fetch` 後驅動 provider 預設路徑 `getReadonlyQuoteCandidate("3019")`（無 options）、明確 `{ dryRunLiveFetch: false }`、以及 unsupported symbol，皆斷言 fetch call count = 0 且回 safe scaffold/disabled/non-operational candidate；測試後還原 fetch、`real_network_used=false`。見 [docs/limited-live-fetch-default-no-fetch-boundary.md](docs/limited-live-fetch-default-no-fetch-boundary.md)；`npm run test:limited-live-fetch-default-no-fetch-boundary`；此 validator 已於下一版納入 safety chain（見下）。）

Default no-fetch boundary validation for limited live fetch is now part of the safety chain as an offline deterministic default-runtime guard. It verifies zero fetch calls unless dryRunLiveFetch is explicitly enabled and does not include manual smoke.（**Default No-Fetch Safety Chain Integration**：`test:limited-live-fetch-default-no-fetch-boundary` 已納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 20 增加到 **21**；default no-fetch check critical=true / passed=true / decision OFFLINE_DETERMINISTIC_DEFAULT_NO_FETCH_OK / mode OFFLINE_DETERMINISTIC_DEFAULT_RUNTIME_PATH / offline=true / deterministic=true / defaultRuntimePath=true / dryRunLiveFetchDefault=false / defaultPathFetchCallCount=0 / explicitDryRunFalseFetchCallCount=0 / unsupportedSymbolDefaultPathSafeFallback=true / realNetworkUsed=false / fetchMockRestored=true / liveFetchPerformed=false / smokeInvoked=false / approvedSymbol=3019 / approvedChannel=tse_3019.tw / productionDataSwitchAllowed·operationalUseAllowed·productionReady false。manual smoke script 仍 **不**納入 safety chain；observation validators 與 deterministic clock validator 維持 standalone，本版不新增進 safety-chain。）

Limited live fetch safety-chain inventory validation freezes the current 21-check safety chain composition, script order, and manual-only exclusions. It remains standalone and performs no live fetch.（凍結 `test:safety-chain` 的 21 個 chain validator 順序（+ `test:safety-chain-ci-guard` last）與 CHAIN_SPECS sourceScript 順序，並驗證 golden / mock fetch / default no-fetch 在 chain 內，smoke / observation round1·2 / deterministic clock / inventory 自身在 chain 外；任何誤增、誤刪、誤排序、誤把 manual-only script 放進 chain 都會被攔。見 [docs/limited-live-fetch-safety-chain-inventory.md](docs/limited-live-fetch-safety-chain-inventory.md)；`npm run test:limited-live-fetch-safety-chain-inventory`；此 validator 維持 standalone，**不**納入 `npm run test:safety-chain`，safety chain 仍 21 checks。）

Limited live fetch timeout boundary validation verifies safe fallback when the mocked TWSE request times out or aborts. It remains standalone, performs no real network request, and does not include manual smoke.（mock `globalThis.fetch` + 把 provider 的 3000ms abort `setTimeout` 假化為 microtask 觸發，讓 AbortController 真的 abort（不等真 3000ms），signal-aware mock 以 AbortError reject；另含 immediate AbortError case。兩者皆回 safe disabled / non-operational candidate、deterministic receivedAt；測試後還原 fetch 與 setTimeout、`real_network_used=false`。見 [docs/limited-live-fetch-timeout-boundary.md](docs/limited-live-fetch-timeout-boundary.md)；`npm run test:limited-live-fetch-timeout-boundary`；此 validator 已於下一版納入 safety chain（見下）。）

Timeout boundary validation for limited live fetch is now part of the safety chain as an offline deterministic timeout/abort fallback guard. It performs no real network request and does not include manual smoke.（**Timeout Boundary Safety Chain Integration**：`test:limited-live-fetch-timeout-boundary` 已納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 21 增加到 **22**；timeout check critical=true / passed=true / decision OFFLINE_DETERMINISTIC_TIMEOUT_OK / mode OFFLINE_DETERMINISTIC_TIMEOUT_BOUNDARY / offline=true / deterministic=true / timeoutBoundary=true / timeoutMs=3000 / maxRetries=0 / realNetworkUsed=false / fetchMockRestored=true / setTimeoutRestored=true / timeoutAbortSafeFallback=true / receivedAtDeterministic=true / liveFetchPerformed=false / smokeInvoked=false / approvedSymbol=3019 / approvedChannel=tse_3019.tw / productionDataSwitchAllowed·operationalUseAllowed·productionReady false。inventory validator 凍結值已同步更新為 22；manual smoke script 仍 **不**納入 safety chain；observation validators、deterministic clock、inventory validator 維持 standalone，本版不新增進 safety-chain。）

Staging shadow runtime scaffold exists but remains SCAFFOLD_ONLY_NOT_CONNECTED / NO_GO / no live fetch：新增 `services/market-data/`（public-quote-provider.types.ts、yahoo-readonly-provider.ts、twse-tpex-verification-provider.ts，皆 scaffold-only / NOT_CONNECTED，getReadonlyQuoteCandidate 不 fetch、不讀 env、不連線）、`use-cases/war-room/build-shadow-runtime-comparison.ts`（buildStagingShadowRuntimeContract / buildShadowRuntimeComparison …）、`scripts/validate-staging-shadow-runtime-scaffold.ts`、`docs/staging-readonly-shadow-runtime-scaffold.md`。contractVersion = STAGING_SHADOW_RUNTIME_SCAFFOLD、decision = NO_GO、shadowRuntimeAllowed/liveFetchAllowed/envReadAllowed/supabaseConnectionAllowed/apiRouteAllowed/portfolioApiSwitchAllowed/productionReady = false、serviceRoleForbidden = true；下一階段需再次 owner approval 才能 limited live fetch dry-run。`npm run test:staging-shadow-runtime-scaffold`。**Staging Shadow Runtime Scaffold Safety Chain Integration：已把 `test:staging-shadow-runtime-scaffold` 納入 `npm run test:safety-chain` 與 V73 Safety Chain CI Guard（CHAIN_SPECS / REQUIRED_SCRIPTS），guard 覆蓋 checks 由 15 增加到 16；scaffold check critical=true / passed=true / decision NO_GO / mode SCAFFOLD_ONLY_NOT_CONNECTED / liveFetchAllowed·envReadAllowed·supabaseConnectionAllowed·portfolioApiSwitchAllowed·productionReady 全 false / serviceRoleForbidden true，GitHub Actions Safety Chain workflow 自動涵蓋。**

V74 GitHub Actions Safety Chain Workflow：把 V73 的 Safety Chain CI Guard 實際接到 GitHub Actions，讓每次 push / pull_request 到 main 自動跑 npm ci → npm run test:safety-chain → npm run build，防止任何 commit 偷翻安全旗標。新增 `.github/workflows/safety-chain.yml`（name: Safety Chain、on push/pull_request main、permissions contents: read、job safety-chain on ubuntu-latest：checkout → setup-node → npm ci → test:safety-chain → build）、`docs/github-actions-safety-chain.md`、`scripts/validate-github-actions-safety-chain.ts`。CI guard only / fixture/mock safe mode：no Supabase secrets、no Vercel deploy、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY；workflow 不含 secrets / SUPABASE / VERCEL_TOKEN / deploy / curl / wget / fetch / service_role / write-all / contents: write / workflow_dispatch / schedule / production switch。`npm run test:github-actions-safety-chain`。

V73 Safety Chain CI Guard：一支 spec-only / deterministic 的 single CI guard，用 contract + validator 彙總檢查 V60–V72 全部安全鏈狀態，確保所有既有 validator 都能通過，且任何 commit 都不能偷翻 NO_GO / false / fixture-only / not connected / no API route / no Supabase / no env / no fetch / no production switch 等安全旗標。新增 `use-cases/war-room/safety-chain-ci-guard-contract.ts`（SafetyChainCiGuardCheck / SafetyChainCiGuardResult / SafetyChainCiGuardContract / SafetyChainCiGuardValidation）、`use-cases/war-room/build-safety-chain-ci-guard-contract.ts`（in-process 呼叫 V60–V72 全部 builder 並掃描安全旗標）、`docs/safety-chain-ci-guard.md`、`scripts/validate-safety-chain-ci-guard.ts`。guardMode = SPEC_ONLY_CI_GUARD、decision = READY_FOR_CI_GUARD（READY_FOR_UI_REVIEW is not production ready）；涵蓋 13 支 V60–V72 script，allCriticalPassed true、allNoGoLocksPreserved true、allRuntimeFlagsFalse true、allOperationalUseBlocked true、productionSwitchStillBlocked true、productionReady false。新增 `npm run test:safety-chain-ci-guard` 與一鍵 `npm run test:safety-chain`；`/system/safety` 新增 Safety Chain CI Guard 卡片。仍是 spec-only / deterministic：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。

V72 Ledger Integrity Rollup & Safety Gate Dashboard：把 V70 evidence ledger 與 V71 transition / source contract integrity 結果收斂成 ledger integrity rollup，並在 /system/safety 顯示成更清楚的安全總覽。新增 `use-cases/war-room/ledger-integrity-rollup-contract.ts`（LedgerIntegrityHealthStatus / LedgerIntegrityRollupItem / LedgerSafetyGateBlocker / LedgerIntegrityRollup / LedgerIntegrityRollupValidation）、`use-cases/war-room/ledger-integrity-rollup-engine.ts`（純函式 buildLedgerIntegrityRollup / deriveLedgerHealthStatus / deriveLedgerSafetyGateBlockers / validateLedgerIntegrityRollup）、`use-cases/war-room/build-ledger-integrity-rollup-contract.ts`、`docs/ledger-integrity-rollup.md`、`scripts/validate-ledger-integrity-rollup.ts`。rollupMode = SPEC_ONLY_SAFETY_GATE、decision = NO_GO；sourceIntegrityOk true、allSourceContractsExist true、allEvidencePending true、allTransitionsPreviewOnly true、actualLedgerMutated false；safetyGateBlockers 含 MANUAL_SIGNOFF_PENDING / EVIDENCE_PENDING / STAGING_LOCKED / REAL_QUOTE_LOCKED / PRODUCTION_SWITCH_LOCKED 且全部 resolved false；`/system/safety` 新增 Ledger Integrity Rollup card，`/holdings` 候選池加註 Ledger integrity rollup: NO_GO / SPEC_ONLY_SAFETY_GATE。仍是 spec-only / deterministic：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:ledger-integrity-rollup`。

V71 Evidence Ledger Transition Engine & Source Contract Integrity：描述 V70 evidence ledger 未來如何在人工 evidence 被提供與接受時重新計算狀態（PREVIEW only，不真的變更 ledger），並檢查 V70 ledger item 的 sourceContracts 是否真的指向現有 contract/doc 檔案。新增 `use-cases/war-room/evidence-ledger-transition-contract.ts`（EvidenceTransitionInput / EvidenceTransitionResult / EvidenceLedgerRecalculationResult / EvidenceSourceContractIntegrityItem / EvidenceLedgerTransitionContract / EvidenceLedgerTransitionValidation）、`use-cases/war-room/evidence-ledger-transition-engine.ts`（純函式 applyEvidenceTransitionPreview / recalculateEvidenceCategories / recalculateLedgerDecision / validateEvidenceSourceContractIntegrity）、`use-cases/war-room/build-evidence-ledger-transition-contract.ts`、`docs/evidence-ledger-transition-engine.md`、`scripts/validate-evidence-ledger-transition.ts`。transitionMode = SPEC_ONLY_PREVIEW_NOT_CONNECTED、decision = NO_GO、ledgerDecisionAfterPreview NO_GO、actualLedgerMutated false；transition preview 對 20 項 evidence 皆 PREVIEW_ONLY 且不變更 ledger；sourceContractIntegrityItems 覆蓋所有 sourceContracts 且 sourceContractExists 全部 true；`/system/safety` 新增 Evidence Ledger Transition Engine card，`/holdings` 候選池加註 Evidence transition engine: NO_GO / PREVIEW_ONLY。仍是 spec-only / deterministic：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:evidence-ledger-transition`。

V70 Unified Connection Evidence Ledger：把 V64–V69 分散描述的 evidence requirements 收斂成單一真相表，追蹤未來接 staging read-only / real quote / shadow comparison / production switch 前必須完成哪些人工 evidence。新增 `use-cases/war-room/unified-connection-evidence-ledger-contract.ts`（UnifiedConnectionEvidenceItem / UnifiedConnectionEvidenceCategory / UnifiedConnectionEvidenceLedger / UnifiedConnectionEvidenceDependency / UnifiedConnectionEvidenceLedgerValidation）、`use-cases/war-room/build-unified-connection-evidence-ledger-contract.ts`、`docs/unified-connection-evidence-ledger.md`、`scripts/validate-unified-connection-evidence-ledger.ts`。ledgerMode = SPEC_ONLY_PENDING_EVIDENCE，decision = NO_GO；20 個 evidence items 全部 evidenceStatus PENDING / evidenceProvided false / evidenceAccepted false / actualEvidenceAttached false；stagingConnectionAllowed false、realQuoteConnectionAllowed false、productionSwitchAllowed false、manualSignoffCompleted false；`/system/safety` 新增 Unified Connection Evidence Ledger card，`/holdings` 候選池加註 Connection evidence ledger: NO_GO / PENDING。仍是 spec-only / deterministic contract：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:unified-connection-evidence-ledger`。

V69 Downgraded Trade Plan UI Behavior：把 V68 downgrade matrix 真正反映到 /holdings 的 candidate / structured trade plan UI 行為（TradePlanVerificationDowngradeResult → V69 UI behavior → CandidateTradePlan card display state）。新增 `use-cases/war-room/downgraded-trade-plan-ui-behavior-contract.ts`（DowngradedTradePlanUiState / DowngradedTradePlanUiVisibility / DowngradedTradePlanUiWarning / DowngradedTradePlanUiBehaviorContract / DowngradedTradePlanUiBehaviorValidation）、`use-cases/war-room/downgraded-trade-plan-ui-behavior-engine.ts`（純函式 buildDowngradedTradePlanUiState / resolveTradePlanVisibility / resolveTradePlanWarning / validateDowngradedTradePlanUiState）、`use-cases/war-room/build-downgraded-trade-plan-ui-behavior-contract.ts`、`docs/downgraded-trade-plan-ui-behavior.md`、`scripts/validate-downgraded-trade-plan-ui-behavior.ts`。behaviorMode = FIXTURE_ONLY_NOT_CONNECTED；依 TradePlanDisplayMode（SHOW_FIXTURE_WITH_WARNING / SHOW_OBSERVATION_ONLY / SHOW_BLOCKED_CONFLICT / SHOW_BLOCKED_MISSING_DATA / SHOW_BLOCKED_STALE_DATA / SHOW_BLOCKED_UNAUTHORIZED / HIDE_OPERATIONAL_LEVELS）決定承接區 / 目標區 / 風報比顯示或隱藏與 warning / actionLabel；所有 uiState observationOnly true、operationalUseAllowed false、actionLabel 無買進語氣、顯示「這不是買賣指令」。`/holdings` 候選池 structured trade plan card 依 uiState 顯示/隱藏，`/system/safety` 新增 Downgraded Trade Plan UI Behavior card。仍是 fixture-only / deterministic：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:downgraded-trade-plan-ui-behavior`。

V68 Conflict Resolution to Trade Plan Verification Downgrade Matrix：建立 spec-only / deterministic contract 與 pure function engine，描述 V67 conflict resolution result 如何回寫或影響 V63 CandidateTradePlan 的 verificationStatus、UI display mode、operational block reason、observation-only 狀態與 trade plan visibility（QuoteSourceConflictResolutionResult → TradePlanVerificationDowngrade → CandidateTradePlan UI state）。新增 `use-cases/war-room/conflict-to-trade-plan-verification-contract.ts`（TradePlanVerificationStatus / TradePlanDisplayMode / TradePlanVerificationDowngradeRule / TradePlanVerificationDowngradeResult / ConflictToTradePlanVerificationMatrix / ConflictToTradePlanVerificationValidation）、`use-cases/war-room/conflict-to-trade-plan-verification-engine.ts`（純函式 mapConflictResultToVerificationStatus / mapConflictResultToDisplayMode / buildTradePlanDowngradeResult / validateTradePlanDowngradeConsistency）、`use-cases/war-room/build-conflict-to-trade-plan-verification-contract.ts`、`docs/conflict-to-trade-plan-verification.md`、`scripts/validate-conflict-to-trade-plan-verification.ts`。matrixMode = SPEC_ONLY_NOT_CONNECTED；VERIFIED future-only（目前 sample 不使用）；所有 downgrade result observationOnly true、operationalUseAllowed false；`/system/safety` 新增 Conflict to Trade Plan Verification Downgrade card，`/holdings` 候選池加註 Trade plan verification: SPEC_ONLY_NOT_CONNECTED。仍是 fixture-only / deterministic contract：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:conflict-to-trade-plan-verification`。

V67 Real Quote Source Conflict Resolution Policy：建立 spec-only / deterministic 的 conflict resolution policy 與 pure function engine，描述未來多個授權來源同時提供同一欄位時如何處理來源衝突、缺值、延遲、時間戳不一致、優先權、降級與禁止操作狀態（multiple source candidate values → deterministic conflict policy → selected source / degraded status / blocked operational use）。新增 `use-cases/war-room/real-quote-source-conflict-resolution-policy-contract.ts`（QuoteSourceCandidateValue / QuoteSourceConflictRule / QuoteSourceConflictResolutionResult / QuoteSourceConflictResolutionPolicy / QuoteSourceConflictResolutionValidation）、`use-cases/war-room/real-quote-source-conflict-resolution-engine.ts`（純函式 rankSourceCandidateValues / detectQuoteSourceConflict / resolveQuoteSourceConflict / buildConflictResolutionResult）、`use-cases/war-room/build-real-quote-source-conflict-resolution-policy-contract.ts`、`docs/real-quote-source-conflict-resolution-policy.md`、`scripts/validate-real-quote-source-conflict-resolution-policy.ts`。policyMode = SPEC_ONLY_NOT_CONNECTED，source priority 沿用 V66 conflictPriorityRank；resolution 永遠 degraded、operationalUseAllowed false；`/system/safety` 新增 Conflict Resolution Policy card，`/holdings` 候選池加註 Conflict policy: SPEC_ONLY_NOT_CONNECTED。仍是 fixture-only / deterministic contract：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:real-quote-source-conflict-resolution-policy`。

V66 Authorized Real Quote Field Catalog & Source Boundary：建立 spec-only / deterministic 的 authorized real quote field catalog，描述 V65 expectedRealFields 未來允許從哪些資料來源取得、目前授權與連線狀態、缺資料與來源衝突行為，以及進入 staging read-only / shadow comparison 前需要哪些 evidence。新增 `use-cases/war-room/authorized-real-quote-field-catalog-contract.ts`（AuthorizedRealQuoteSourceCandidate / AuthorizedRealQuoteFieldCatalogItem / AuthorizedRealQuoteFieldCatalog / AuthorizedRealQuoteSourceBoundary / AuthorizedRealQuoteFieldCatalogValidation）、`use-cases/war-room/build-authorized-real-quote-field-catalog-contract.ts`、`docs/authorized-real-quote-field-catalog.md`、`scripts/validate-authorized-real-quote-field-catalog.ts`。sourceCatalogMode = SPEC_ONLY_NOT_CONNECTED，6 個 source candidate（Yahoo Finance Taiwan / TWSE / TPEx / Goodinfo / Supabase staging read-only / Broker import）全部 NOT_CONNECTED + NOT_AUTHORIZED 或 PENDING_MANUAL_REVIEW、runtimeEnabled false、fetchAllowed false；field catalog 覆蓋 V65 expectedRealFields；`/system/safety` 新增 Authorized Real Quote Field Catalog card，`/holdings` 候選池加註 Authorized source catalog: SPEC_ONLY_NOT_CONNECTED。仍是 fixture-only / deterministic contract：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:authorized-real-quote-field-catalog`。

V65 Descriptor-to-Real Quote Mapping Readiness Matrix：建立 spec-only / deterministic 的 mapping readiness matrix，描述未來真實行情欄位（futureRealQuoteField）如何對應到 V64 fixture descriptor 欄位，再流入 V63 structured trade plan 欄位。新增 `use-cases/war-room/descriptor-to-real-quote-mapping-contract.ts`（RealQuoteExpectedField / DescriptorFieldMapping / TradePlanFieldMapping / DescriptorToRealQuoteMappingItem / DescriptorToRealQuoteMappingMatrix / DescriptorToRealQuoteMappingReadinessValidation）、`use-cases/war-room/build-descriptor-to-real-quote-mapping-contract.ts`、`docs/descriptor-to-real-quote-mapping-readiness.md`、`scripts/validate-descriptor-to-real-quote-mapping.ts`。每個 V64 descriptor 與每個 V63 trade plan 都有一筆 mapping item，descriptor / trade plan / mapping symbol 完全一致；realMappingStatus = NOT_CONNECTED、mappingReadiness = SPEC_DEFINED_NOT_CONNECTED、realQuoteConnected false、manualSignoffRequired true、manualSignoffCompleted false、stagingReadOnlyRequired true、stagingReadOnlyConnected false、shadowComparisonRequired true、shadowComparisonCompleted false、productionSwitchAllowed false、operationalUseAllowed false；`/system/safety` 新增 Descriptor-to-Real Quote Mapping Readiness card，`/holdings` 候選池加註 Real quote mapping: SPEC_DEFINED_NOT_CONNECTED。仍是 fixture-only / deterministic contract：no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:descriptor-to-real-quote-mapping`。

V64 Trade Plan Fixture Source Descriptor & Mapping Boundary：把 V63 structured trade plan 用的 fixture price levels（原本散落在 V63 builder 的 PRICE_LEVELS）抽成獨立的 fixture source descriptor contract，並建立 future real quote mapping boundary。新增 `use-cases/war-room/candidate-price-level-fixture-source-contract.ts`（CandidatePriceLevelFixtureSource / CandidatePriceLevelDescriptor / CandidatePriceLevelMappingBoundary / CandidatePriceLevelFixtureSourceBundle / CandidatePriceLevelFixtureSourceValidation）、`use-cases/war-room/build-candidate-price-level-fixture-source-contract.ts`、`docs/candidate-price-level-fixture-source.md`、`scripts/validate-candidate-price-level-fixture-source.ts`。V63 builder 改由 descriptor 取得價位（不再手寫 PRICE_LEVELS），descriptor 與 trade plan 價位完全一致；UI 顯示 realMappingStatus = NOT_CONNECTED、未連真實報價、fixture 區間不可作為正式操作依據，`/system/safety` 新增 spec-only mapping boundary health card。仍是 fixture-only / deterministic contract：realMappingStatus NOT_CONNECTED、futureRealSourceAllowed false、operationalUseAllowed false、manualSignoffRequired true、manualSignoffCompleted false、stagingReadOnlyRequired true、productionSwitchAllowed false；no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。`npm run test:candidate-price-level-fixture-source`。

V63 Structured Buy Zone & Risk/Reward Contract：把 candidate 的 pullbackBuyZone / riskRewardRatio / entryStrategy 從字串敘述升級為結構化 deterministic contract，讓承接區 / 失效防守區 / 目標區 / 風報比可被 validator 檢查一致性。新增 `use-cases/war-room/structured-candidate-trade-plan-contract.ts`（StructuredBuyZone / StructuredRiskReward / StructuredEntryStrategy / CandidateTradePlan / CandidateTradePlanValidation）、`use-cases/war-room/build-structured-candidate-trade-plan-contract.ts`、`docs/structured-candidate-trade-plan.md`、`scripts/validate-structured-candidate-trade-plan.ts`，並在每日候選池顯示結構化承接區 / 失效防守區 / 目標觀察區。validator 檢查 buyZone.lower <= buyZone.upper、stopLossUpper < buyZone.lower、targetLower > buyZone.upper、rewardRiskRatio > 0 且可由 upside/downside 推導、sourceType fixture_mock、operationalUseAllowed false、isPosition false、pnlComputable false、no shares、no cost basis、observation only（not buy/sell command）、每個 trade plan 對應一個 V62 candidate 且 score/grade/pool 與引擎一致。仍是 fixture-only / deterministic contract / validator / UI structure：no Supabase connection、no env read、no DB write、no real market data、no /api/portfolio switch、no auto order、not PRODUCTION_READY。`npm run test:structured-candidate-trade-plan`。
...
- 把 V73 的 Safety Chain CI Guard 實際接到 GitHub Actions：每次 push / pull_request 到 main 自動執行 npm ci → npm run test:safety-chain → npm run build，防止任何 commit 偷翻 NO_GO / fixture-only / not connected / no API route / no Supabase / no env / no fetch / no production switch 等安全旗標。本版是 CI guard only / fixture/mock safe mode；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase secrets、no Vercel deploy、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不引入任何 secret、不建立 deployment write action、不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `.github/workflows/safety-chain.yml`：name: Safety Chain；on push / pull_request branches: [main]；permissions contents: read（唯讀）；job safety-chain runs-on ubuntu-latest，steps = actions/checkout → actions/setup-node（node 20、cache npm）→ npm ci → npm run test:safety-chain → npm run build。workflow 不含 secrets / SUPABASE / VERCEL_TOKEN / deploy / curl / wget / fetch / service_role / write-all / contents: write / workflow_dispatch / schedule / /api/portfolio / PRODUCTION_READY。
- 新增 `docs/github-actions-safety-chain.md`：workflow 結構、禁止項目、validator 說明、安全邊界。
- 新增 `scripts/validate-github-actions-safety-chain.ts`：8 gates（required_files / workflow_content / workflow_triggers / workflow_forbidden / doc_phrases / package_checks / readme_checks / no_deploy_artifacts），驗證 workflow 含 name: Safety Chain、push+pull_request main、contents: read、npm ci / test:safety-chain / build，並掃描 workflow 不含 secrets / supabase / vercel / deploy / curl / wget / fetch / service_role / write-all / contents: write / workflow_dispatch / schedule / /api/portfolio / PRODUCTION_READY，且未新增 deploy / vercel / production workflow。
- 新增 `npm run test:github-actions-safety-chain`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未引入 secret；未建立 deployment action；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：GitHub Actions Safety Chain Workflow（[docs/github-actions-safety-chain.md](docs/github-actions-safety-chain.md)）。

### V73

Safety Chain CI Guard：

- 一支 spec-only / deterministic single CI guard，用 contract + validator 彙總檢查 V60–V72 全部安全鏈狀態：確保 13 支既有 validator 對應的 builder 都維持鎖定 decision，且任何 commit 都不能偷翻 NO_GO / fixture-only / not connected / no runtime / no API route / no Supabase / no env / no fetch / no production switch 等安全旗標。本版仍是 spec-only / deterministic CI guard contract + validator + package script；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/safety-chain-ci-guard-contract.ts`：型別 SafetyChainCiGuardCheck、SafetyChainCiGuardResult、SafetyChainCiGuardContract、SafetyChainCiGuardValidation；guardMode = SPEC_ONLY_CI_GUARD、decision = READY_FOR_CI_GUARD。
- 新增 `use-cases/war-room/build-safety-chain-ci-guard-contract.ts`：pure deterministic builder，in-process（無 child process、無 spawn）呼叫 V60–V72 全部 13 支 builder，逐一確認 decision 仍為鎖定值（V60–V69 READY_FOR_UI_REVIEW、V70–V72 NO_GO）並掃描每個 bundle 是否有 realDataConnected / runtimeCreated / apiRouteCreated / envReadPerformed / fetchPerformed / supabaseConnected / databaseWritePerformed / portfolioApiSwitched / productionReady / productionTradingReady / operationalUseAllowed / productionSwitchAllowed / buySellCommandGenerated / autoOrderRequested 被翻成 true；輸出 13 個 critical check 與 result（allCriticalPassed / allNoGoLocksPreserved / allRuntimeFlagsFalse / allOperationalUseBlocked / productionSwitchStillBlocked true）；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Safety Chain CI Guard 卡片：chain checks passed/total、allCriticalPassed、allRuntimeFlagsFalse、allNoGoLocksPreserved、allOperationalUseBlocked、productionSwitchStillBlocked、productionReady false）。
- 新增 `scripts/validate-safety-chain-ci-guard.ts`：8 gates，import guard builder 實際呼叫，驗證 contractVersion / specName / guardMode / decision = READY_FOR_CI_GUARD、checks 非空且覆蓋 V60–V72 13 支 script、每個 check critical=true 且 passed=true、result 全部 lock 旗標、contract-level 連線旗標 false，UI 含必要字串，並**大小寫不敏感**掃描新 contract/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:safety-chain-ci-guard` 與一鍵 `npm run test:safety-chain`（依序跑 V60–V72 全部 validator + V73 guard）。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Safety Chain CI Guard（[docs/safety-chain-ci-guard.md](docs/safety-chain-ci-guard.md)）。

### V72

Ledger Integrity Rollup & Safety Gate Dashboard：

- 把 V70 evidence ledger 與 V71 transition / source contract integrity 結果收斂成 ledger integrity rollup，並在 /system/safety 顯示成更清楚的安全總覽。本版仍是 spec-only / deterministic rollup + safety gate dashboard + validator；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** 不真的接受任何 production evidence。no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/ledger-integrity-rollup-contract.ts`：型別 LedgerIntegrityHealthStatus（HEALTHY_SPEC_ONLY / PENDING_EVIDENCE / BLOCKED_NO_GO / MISSING_SOURCE_CONTRACT / PREVIEW_ONLY_LOCKED / PRODUCTION_LOCKED）、LedgerIntegrityRollupItem、LedgerSafetyGateBlocker、LedgerIntegrityRollup、LedgerIntegrityRollupValidation；rollupMode = SPEC_ONLY_SAFETY_GATE、decision / ledgerDecision / transitionDecision = NO_GO。
- 新增 `use-cases/war-room/ledger-integrity-rollup-engine.ts`：純函式 buildLedgerIntegrityRollup（合併 V70 evidence item + V71 integrity / preview）、deriveLedgerHealthStatus、deriveLedgerSafetyGateBlockers（產出 MANUAL_SIGNOFF_PENDING / EVIDENCE_PENDING / STAGING_LOCKED / REAL_QUOTE_LOCKED / PRODUCTION_SWITCH_LOCKED 五類 blocker，全部 resolved false）、validateLedgerIntegrityRollup；無副作用、無 I/O、不連線、不讀 env、不 fetch、不讀時鐘、不寫資料。
- 新增 `use-cases/war-room/build-ledger-integrity-rollup-contract.ts`：pure deterministic builder，輸出 20 筆 rollupItems（每筆 sourceContractExists true / operationalUseAllowed false）與 5 個 safety gate blockers；sourceIntegrityOk true、allSourceContractsExist true、allEvidencePending true、allTransitionsPreviewOnly true、actualLedgerMutated false；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Ledger Integrity Rollup card：rollup items count、sourceIntegrityOk / allSourceContractsExist、safety gate blockers count、allEvidencePending / allTransitionsPreviewOnly / actualLedgerMutated、staging / realQuote / production connection false、productionReady false）、`components/war-room/daily-candidate-pools.tsx`（加註 Ledger integrity rollup: NO_GO / SPEC_ONLY_SAFETY_GATE、source contracts 完整但 evidence 全部 pending）、`components/war-room/war-room-operational-layout.tsx`（import rollup builder 並傳入候選池）。
- 新增 `scripts/validate-ledger-integrity-rollup.ts`：8 gates，import builder 與 engine 實際呼叫，驗證 contractVersion / specName / rollupMode / decision / ledgerDecision / transitionDecision = NO_GO、sourceIntegrityOk / allSourceContractsExist / allEvidencePending / allTransitionsPreviewOnly true、所有連線旗標 false、rollupItems 非空且覆蓋 V70 evidence、每筆 sourceContractExists true 且 operationalUseAllowed false、safetyGateBlockers 非空且全部 resolved false 且含五類必要 blockerType，UI 含必要字串，並**大小寫不敏感**掃描新 contract/engine/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:ledger-integrity-rollup`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Ledger Integrity Rollup & Safety Gate Dashboard（[docs/ledger-integrity-rollup.md](docs/ledger-integrity-rollup.md)）。

### V71

Evidence Ledger Transition Engine & Source Contract Integrity：

- 描述 V70 evidence ledger 未來如何在人工 evidence 被提供與接受時重新計算狀態（PREVIEW only，actualLedgerMutated false），並檢查 V70 ledger item 的 sourceContracts 是否真的指向現有 contract/doc 檔案。本版仍是 spec-only / deterministic transition engine + source contract integrity validator + UI health card；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** 不真的接受任何 production evidence。no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/evidence-ledger-transition-contract.ts`：型別 EvidenceTransitionInput、EvidenceTransitionResult、EvidenceLedgerRecalculationResult、EvidenceSourceContractIntegrityItem、EvidenceLedgerTransitionContract、EvidenceLedgerTransitionValidation；transitionMode = SPEC_ONLY_PREVIEW_NOT_CONNECTED、decision = NO_GO、ledgerDecisionAfterPreview = NO_GO、actualLedgerMutated false。
- 新增 `use-cases/war-room/evidence-ledger-transition-engine.ts`：純函式 applyEvidenceTransitionPreview（preview-only，不變更 ledger）、recalculateEvidenceCategories、recalculateLedgerDecision（未完成全部 evidence 前回 NO_GO）、validateEvidenceSourceContractIntegrity（file existence 由注入 predicate 提供，保持純函式）。
- 新增 `use-cases/war-room/build-evidence-ledger-transition-contract.ts`：pure deterministic builder，對 V70 ledger 20 項 evidence 各產生一筆 PREVIEW_ONLY transition result（即使 proposed accepted，actualLedgerMutated 仍 false、ledger decision 仍 NO_GO），recalculate 統計（total 20 / completed 0 / pending 20）、source contract integrity（展開所有 sourceContracts 並以 node:fs 檢查存在，全部 sourceContractExists true）；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不讀 env、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Evidence Ledger Transition Engine card：preview results count、source contract integrity count、all source contracts exist、ledgerDecisionAfterPreview NO_GO、staging / realQuote / production connection false、productionReady false）、`components/war-room/daily-candidate-pools.tsx`（加註 Evidence transition engine: NO_GO / PREVIEW_ONLY、即使 preview 單項 evidence 真實行情與 staging 連線仍維持鎖定）、`components/war-room/war-room-operational-layout.tsx`（import transition builder 並傳入候選池）。
- 新增 `scripts/validate-evidence-ledger-transition.ts`：8 gates，import builder 與 engine 實際呼叫，驗證 contractVersion / specName / transitionMode / decision NO_GO / ledgerDecisionAfterPreview NO_GO / actualLedgerMutated false、preview results 全 PREVIEW_ONLY 且不變更 ledger、recalculate counts 正確且 decision NO_GO、preview 後 ledger 仍 PENDING、sourceContractIntegrityItems 覆蓋所有 sourceContracts 且全部存在（並與實際 filesystem 交叉比對）、kind 為 contract/doc、referencedVersion 含 V64–V70、所有連線旗標 false，UI 含必要字串，並**大小寫不敏感**掃描新 contract/engine/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:evidence-ledger-transition`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Evidence Ledger Transition Engine（[docs/evidence-ledger-transition-engine.md](docs/evidence-ledger-transition-engine.md)）。

### V70

Unified Connection Evidence Ledger：

- 把 V64 / V65 / V66 / V67 / V68 / V69 分散描述的 evidence requirements 收斂成單一真相表，追蹤未來接 staging read-only / real quote / shadow comparison / production switch 前必須完成哪些人工 evidence（manual sign-off / source authorization / staging read-only review / RLS select-only / service role not in app runtime / no write permission / no buy-sell / no auto order / shadow comparison / rollback runbook / kill switch / production switch approval）。本版仍是 spec-only / deterministic contract / validator / optional UI health card；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/unified-connection-evidence-ledger-contract.ts`：型別 UnifiedConnectionEvidenceItem、UnifiedConnectionEvidenceCategory、UnifiedConnectionEvidenceLedger、UnifiedConnectionEvidenceDependency、UnifiedConnectionEvidenceLedgerValidation；ledgerMode = SPEC_ONLY_PENDING_EVIDENCE、decision = NO_GO。
- 新增 `use-cases/war-room/build-unified-connection-evidence-ledger-contract.ts`：pure deterministic builder，輸出 20 個 evidence items（OWNER_MANUAL_SIGNOFF / SOURCE_AUTHORIZATION_YAHOO / TWSE / TPEX / GOODINFO / BROKER_IMPORT / STAGING_SUPABASE_PROJECT_CONFIRMATION / STAGING_READONLY_ROLE_CONFIRMATION / RLS_SELECT_ONLY_CONFIRMATION / SERVICE_ROLE_NOT_IN_APP_RUNTIME / WRITE_OPERATION_BLOCK_CONFIRMATION / API_ROUTE_NO_SWITCH_CONFIRMATION / REAL_QUOTE_MAPPING_REVIEW / SOURCE_CONFLICT_POLICY_REVIEW / TRADE_PLAN_DOWNGRADE_UI_REVIEW / SHADOW_COMPARISON_PLAN / SHADOW_COMPARISON_RESULT_PENDING / ROLLBACK_RUNBOOK / KILL_SWITCH_CONFIRMATION / PRODUCTION_SWITCH_FINAL_APPROVAL），全部 evidenceStatus PENDING / evidenceProvided false / evidenceAccepted false / actualEvidenceAttached false；7 個 categories（completedCount 0、pendingCount = itemCount）與 dependency graph；requiredByVersions 引用 V64–V69、sourceContracts 非空；自我驗證後 decision = NO_GO；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Unified Connection Evidence Ledger card：evidence items count / pending / completed 0、staging / realQuote / production connection allowed false、manualSignoffCompleted / actualEvidenceAttached false、realDataConnected / supabaseConnected / productionReady false）、`components/war-room/daily-candidate-pools.tsx`（加註 Connection evidence ledger: NO_GO / PENDING、真實行情與 staging 連線仍需人工 evidence）、`components/war-room/war-room-operational-layout.tsx`（import ledger builder 並傳入候選池）。
- 新增 `scripts/validate-unified-connection-evidence-ledger.ts`：8 gates，import builder 實際呼叫，驗證 contractVersion / specName / ledgerMode / decision = NO_GO、所有連線與 sign-off 旗標 false、20 個 required items 全存在且 PENDING / not provided / not accepted / not attached、categories pendingCount > 0 且 completedCount 0 且正確聚合、requiredByVersions 引用 V64–V69、sourceContracts 非空、dependencies 引用存在 evidenceId，UI 含必要字串，並**大小寫不敏感**掃描新 contract/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:unified-connection-evidence-ledger`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Unified Connection Evidence Ledger（[docs/unified-connection-evidence-ledger.md](docs/unified-connection-evidence-ledger.md)）。

### V69

Downgraded Trade Plan UI Behavior：

- 把 V68 downgrade matrix 真正反映到 /holdings 的 candidate / structured trade plan UI 行為：當 downgrade result 顯示 SOURCE_CONFLICT / STALE_DATA / MISSING_DATA / UNAUTHORIZED_SOURCE / MANUAL_REVIEW_REQUIRED / OBSERVATION_ONLY / BLOCKED 時，UI 依 TradePlanDisplayMode 決定承接區、目標區、風報比與提示文字如何呈現。本版仍是 fixture-only / deterministic UI behavior / validator；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/downgraded-trade-plan-ui-behavior-contract.ts`：型別 DowngradedTradePlanUiState、DowngradedTradePlanUiVisibility、DowngradedTradePlanUiWarning（warningLevel info/warning/danger）、DowngradedTradePlanUiBehaviorContract（behaviorMode = FIXTURE_ONLY_NOT_CONNECTED）、DowngradedTradePlanUiBehaviorValidation；每個 uiState observationOnly true / operationalUseAllowed false / buySellCommandGenerated false / autoOrderRequested false / manualSignoffRequired true / manualSignoffCompleted false / productionSwitchAllowed false。
- 新增 `use-cases/war-room/downgraded-trade-plan-ui-behavior-engine.ts`：純函式 buildDowngradedTradePlanUiState / resolveTradePlanVisibility / resolveTradePlanWarning / validateDowngradedTradePlanUiState；actionLabel 依 mode 給「觀察 / 等待資料確認 / 資料衝突，禁止操作 / 資料不足，等待 / 資料過期，等待更新 / 來源未授權，禁止操作 / 隱藏操作區間」，絕無買進語氣（this is not a buy/sell command）；無副作用、無 I/O、不連線、不讀 env、不 fetch、不讀時鐘、不寫資料。
- 新增 `use-cases/war-room/build-downgraded-trade-plan-ui-behavior-contract.ts`：pure deterministic builder，對每筆 V68 downgrade result 產生 uiState（mapped 到 V63 trade plan），並加入 SHOW_FIXTURE_WITH_WARNING / SHOW_OBSERVATION_ONLY / HIDE_OPERATIONAL_LEVELS 合成情境覆蓋完整 7 種 display mode；自我驗證 observation only / 無買進語氣 / warning 與 mode 一致 / HIDE 隱藏所有區間；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`components/war-room/daily-candidate-pools.tsx`（structured trade plan card 依 uiState 顯示/隱藏 buy zone / target zone / risk reward，顯示 warningTitle / warningMessage / actionLabel / observationOnly / operationalUseAllowed=false / 「這不是買賣指令」）、`components/war-room/war-room-operational-layout.tsx`（import UI behavior builder 並傳入候選池）、`app/system/safety/page.tsx`（新增 spec-only Downgraded Trade Plan UI Behavior card：uiStates count、operationalUseAllowed false、observationOnly true、manualSignoffCompleted false、productionSwitchAllowed false、realDataConnected / fetchPerformed / supabaseConnected false）。
- 新增 `scripts/validate-downgraded-trade-plan-ui-behavior.ts`：9 gates，import builder 與 engine 實際呼叫，驗證每筆 V68 downgrade 都有 uiState、每個 uiState 對應 V63 trade plan、observationOnly / operationalUseAllowed / buy-sell / auto-order / manual sign-off / production switch 旗標、HIDE_OPERATIONAL_LEVELS 隱藏所有區間、blocked/conflict/missing/stale/unauthorized warning 正確且情境皆覆蓋、actionLabel 無買進/進場/加碼/buy/enter 語氣、warningMessage 含不可作為正式操作依據，UI 含必要字串，並**大小寫不敏感**掃描新 contract/engine/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:downgraded-trade-plan-ui-behavior`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Downgraded Trade Plan UI Behavior（[docs/downgraded-trade-plan-ui-behavior.md](docs/downgraded-trade-plan-ui-behavior.md)）。

### V68

Conflict Resolution to Trade Plan Verification Downgrade Matrix：

- 建立 spec-only / deterministic contract 與 pure function engine，描述 V67 conflict resolution result 如何回寫或影響 V63 CandidateTradePlan 的 verificationStatus、UI display mode、operational block reason、observation-only 狀態與 trade plan visibility。本版仍是 fixture-only / deterministic / validator / optional UI health card；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/conflict-to-trade-plan-verification-contract.ts`：型別 TradePlanVerificationStatus（含 FIXTURE_ONLY / NOT_CONNECTED / SOURCE_CONFLICT / STALE_DATA / MISSING_DATA / UNAUTHORIZED_SOURCE / MANUAL_REVIEW_REQUIRED / OBSERVATION_ONLY / BLOCKED / VERIFIED；VERIFIED future-only）、TradePlanDisplayMode、TradePlanVerificationDowngradeRule、TradePlanVerificationDowngradeResult（observationOnly true / operationalUseAllowed false / buySellCommandGenerated false / autoOrderRequested false / requiresManualReview true / manualSignoffRequired true / manualSignoffCompleted false / productionSwitchAllowed false）、ConflictToTradePlanVerificationMatrix、ConflictToTradePlanVerificationValidation。
- 新增 `use-cases/war-room/conflict-to-trade-plan-verification-engine.ts`：純函式 mapConflictResultToVerificationStatus（永不回傳 VERIFIED）、mapConflictResultToDisplayMode、buildTradePlanDowngradeResult、validateTradePlanDowngradeConsistency；無副作用、無 I/O、不連線、不讀 env、不 fetch、不讀時鐘、不寫資料。
- 新增 `use-cases/war-room/build-conflict-to-trade-plan-verification-contract.ts`：pure deterministic builder，對每筆 V67 sampleResolutionResult 產生對應 downgrade（mapped 到 V63 trade plan），並加入 missing / stale / unauthorized 合成情境覆蓋完整 matrix；自我驗證 VERIFIED 不在 sample、display mode 與 signal 一致、全部 observation only / operational use blocked；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Conflict to Trade Plan Verification Downgrade card：downgrade rules / sample downgrades count、operationalUseAllowed false、observationOnly true、manualSignoffCompleted false、productionSwitchAllowed false、realDataConnected / fetchPerformed / supabaseConnected false）、`components/war-room/daily-candidate-pools.tsx`（加註 Trade plan verification: SPEC_ONLY_NOT_CONNECTED、來源衝突或缺值時承接區降級為觀察）、`components/war-room/war-room-operational-layout.tsx`（import downgrade builder 並傳入候選池）。
- 新增 `scripts/validate-conflict-to-trade-plan-verification.ts`：9 gates，import builder 與 engine 實際呼叫，驗證每筆 V67 result 都有 downgrade、每個 downgrade 對應 V63 trade plan、sample 不使用 VERIFIED、conflict 不得 VERIFIED、operationalUseAllowed / observationOnly / buy-sell / auto-order / manual sign-off / production switch 旗標、missing / stale / unauthorized / conflict 的 display mode 一致且情境皆覆蓋、matrix-level 安全旗標，UI 含必要字串，並**大小寫不敏感**掃描新 contract/engine/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:conflict-to-trade-plan-verification`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Conflict Resolution to Trade Plan Verification Downgrade Matrix（[docs/conflict-to-trade-plan-verification.md](docs/conflict-to-trade-plan-verification.md)）。

### V67

Real Quote Source Conflict Resolution Policy：

- 建立 spec-only / deterministic contract 的 real quote source conflict resolution policy 與 pure function engine，描述未來多個授權來源同時提供同一欄位時如何處理來源衝突、缺值、延遲、時間戳不一致、優先權、降級與禁止操作狀態。本版仍是 fixture-only / deterministic conflict resolution / validator / optional UI health card；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/real-quote-source-conflict-resolution-policy-contract.ts`：型別 QuoteSourceCandidateValue（sourceName / fieldName / valueKind / valuePreview / valueTimestamp / isAvailable / isStale / isAuthorized = false / isConnected = false / sourcePriorityRank / verificationStatus / operationalUseAllowed = false）、QuoteSourceConflictRule、QuoteSourceConflictResolutionResult（operationalUseAllowed false / requiresManualReview true / manualSignoffRequired true / manualSignoffCompleted false / productionSwitchAllowed false）、QuoteSourceConflictResolutionPolicy、QuoteSourceConflictResolutionValidation。
- 新增 `use-cases/war-room/real-quote-source-conflict-resolution-engine.ts`：純函式 rankSourceCandidateValues（依 V66 conflictPriorityRank 排序）、detectQuoteSourceConflict（偵測 missing data / stale data / timestamp mismatch / value mismatch / unauthorized source / not connected source）、resolveQuoteSourceConflict（deterministic 降級，operationalUseAllowed 永遠 false）、buildConflictResolutionResult；無副作用、無 I/O、不連線、不讀 env、不 fetch、不讀時鐘、不寫資料。
- 新增 `use-cases/war-room/build-real-quote-source-conflict-resolution-policy-contract.ts`：pure deterministic builder，conflict rules + sample candidate values（sourceName / fieldName 對齊 V66、sourcePriorityRank 由 V66 conflictPriorityRank 推導）+ sample resolution results（全部 degraded、operationalUseAllowed false），並自我驗證 ranking 與六種衝突偵測；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Real Quote Source Conflict Resolution Policy card：conflict rules / sample resolutions count、operationalUseAllowed false、manualSignoffCompleted false、productionSwitchAllowed false、realDataConnected / fetchPerformed / supabaseConnected false）、`components/war-room/daily-candidate-pools.tsx`（加註 Conflict policy: SPEC_ONLY_NOT_CONNECTED、多來源衝突解析尚未接真實資料）、`components/war-room/war-room-operational-layout.tsx`（import policy builder 並傳入候選池）。
- 新增 `scripts/validate-real-quote-source-conflict-resolution-policy.ts`：9 gates，import builder 與 engine 實際呼叫，驗證 ranking 依 V66 conflictPriorityRank、六種衝突偵測、resolve 不允許 operationalUseAllowed=true、所有 sample result operationalUseAllowed / manualSignoffCompleted / productionSwitchAllowed 安全、sample source/field 名稱對齊 V66、catalog-level 安全旗標，UI 含必要字串，並**大小寫不敏感**掃描新 contract/builder/engine 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:real-quote-source-conflict-resolution-policy`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Real Quote Source Conflict Resolution Policy（[docs/real-quote-source-conflict-resolution-policy.md](docs/real-quote-source-conflict-resolution-policy.md)）。

### V66

Authorized Real Quote Field Catalog & Source Boundary：

- 建立 spec-only / deterministic contract 的 authorized real quote field catalog，描述 V65 expectedRealFields 未來允許從哪些資料來源取得、目前授權狀態、連線狀態、缺資料行為、來源衝突行為，以及進入 staging read-only / shadow comparison 前需要哪些 evidence（expectedRealField → authorized source candidate → authorization status → connection status → evidence requirement → mapping readiness）。本版仍是 fixture-only / deterministic contract / validator / optional UI health card；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/authorized-real-quote-field-catalog-contract.ts`：型別 AuthorizedRealQuoteSourceCandidate（sourceName / sourceType / sourceRole / authorizationStatus = NOT_AUTHORIZED|PENDING_MANUAL_REVIEW / connectionStatus = NOT_CONNECTED / runtimeEnabled / fetchAllowed / envRequired / envReadPerformed / apiRouteRequired / apiRouteCreated / supabaseRequired / supabaseConnected / operationalUseAllowed 全 false / evidenceRequiredBeforeUse / conflictPriorityRank / notes）、AuthorizedRealQuoteFieldCatalogItem、AuthorizedRealQuoteFieldCatalog、AuthorizedRealQuoteSourceBoundary、AuthorizedRealQuoteFieldCatalogValidation 與 constants。
- 新增 `use-cases/war-room/build-authorized-real-quote-field-catalog-contract.ts`：pure deterministic builder，列出 6 個 source candidate（Yahoo Finance Taiwan / TWSE / TPEx / Goodinfo / Supabase staging read-only / Broker import，全部 NOT_CONNECTED + NOT_AUTHORIZED|PENDING_MANUAL_REVIEW）與 13 個 field catalog item（覆蓋 V65 expectedRealFields：lastPrice / closePrice / openPrice / highPrice / lowPrice / volume / referenceClose / supportLevel / resistanceLevel / targetModelOutput / dataTimestamp / dataSource / verificationStatus），每個 item 的 acceptableSources 都對應到實際 source candidate；sourceCatalogMode = SPEC_ONLY_NOT_CONNECTED，source boundary 全部 sign-off / staging / shadow / production / service role / write / buy-sell / auto-order 旗標安全；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Authorized Real Quote Field Catalog card：source candidates / field catalog items count、all NOT_CONNECTED / runtimeEnabled false / fetchAllowed false、manualSignoffCompleted / stagingReadOnlyConnected / shadowComparisonCompleted / productionSwitchAllowed / serviceRoleAllowedInAppRuntime / writeOperationsAllowed false）、`components/war-room/daily-candidate-pools.tsx`（加註 Authorized source catalog: SPEC_ONLY_NOT_CONNECTED、尚未授權任何真實行情來源）、`components/war-room/war-room-operational-layout.tsx`（import catalog builder 並傳入候選池）。
- 新增 `scripts/validate-authorized-real-quote-field-catalog.ts`：9 gates，import builder 實際呼叫，驗證 V65 expectedRealFields 全覆蓋、每個 acceptableSource 對應到 source candidate、所有 source NOT_CONNECTED / runtime / fetch / operational off、所有 field item SPEC_DEFINED_NOT_CONNECTED 與 sign-off / staging / shadow / production 旗標、source boundary 與 catalog-level 安全旗標，UI 含必要字串，並**大小寫不敏感**掃描新 contract/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:authorized-real-quote-field-catalog`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Authorized Real Quote Field Catalog & Source Boundary（[docs/authorized-real-quote-field-catalog.md](docs/authorized-real-quote-field-catalog.md)）。

### V65

Descriptor-to-Real Quote Mapping Readiness Matrix：

- 建立 spec-only / deterministic contract 的 mapping readiness matrix，描述未來真實行情欄位如何對應到 V64 fixture descriptor 欄位（futureRealQuoteField → CandidatePriceLevelDescriptor field → CandidateTradePlan field）。本版仍是 fixture-only / deterministic contract / validator / optional UI health card；**不是 real quote connection、不是 Supabase connection、不是 staging dry-run execution、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection / production switch 旗標。
- 新增 `use-cases/war-room/descriptor-to-real-quote-mapping-contract.ts`：型別 RealQuoteExpectedField（fieldName / fieldCategory / required / acceptableSources / dataFreshnessRequirement / validationRule / missingDataBehavior / conflictBehavior / operationalUseAllowed = false）、DescriptorFieldMapping（descriptorFieldName / descriptorFieldType / currentSourceType = "fixture_mock" / futureSourceType = "real_quote_pending" / mappedExpectedRealFields / fallbackBehavior / manualReviewRequired = true）、TradePlanFieldMapping（tradePlanFieldName / derivedFromDescriptorField / deterministicTransform / transformRuntimeEnabled = false / operationalUseAllowed = false）、DescriptorToRealQuoteMappingItem、DescriptorToRealQuoteMappingMatrix、DescriptorToRealQuoteMappingReadinessValidation 與 constants。
- 新增 `use-cases/war-room/build-descriptor-to-real-quote-mapping-contract.ts`：pure deterministic builder，對每個同時具備 V64 descriptor 與 V63 trade plan 的 symbol 產生一筆 mapping item（6 個 descriptorFieldMapping、6 個 tradePlanFieldMapping、8 個 expectedRealField），並逐筆自我驗證（symbol 一致、realQuoteConnected false、realMappingStatus NOT_CONNECTED、mappingReadiness SPEC_DEFINED_NOT_CONNECTED、各 sign-off / staging / shadow / production / operational 旗標、欄位非空、descriptorFieldMapping 對應實際 V64 descriptor 欄位、tradePlanFieldMapping 對應實際 V63 trade plan 欄位）；generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 UI：`app/system/safety/page.tsx`（新增 spec-only Descriptor-to-Real Quote Mapping Readiness card：mapping items valid / mappingReadiness / realMappingStatus / realQuoteConnected / stagingReadOnlyConnected / shadowComparisonCompleted / productionSwitchAllowed / operationalUseAllowed）、`components/war-room/daily-candidate-pools.tsx`（加註 Real quote mapping: SPEC_DEFINED_NOT_CONNECTED、尚未連真實行情，fixture 區間不可作為正式操作依據）、`components/war-room/war-room-operational-layout.tsx`（import mapping builder 並傳入候選池）。
- 新增 `scripts/validate-descriptor-to-real-quote-mapping.ts`：9 gates，import builder 實際呼叫，驗證每個 V64 descriptor / V63 trade plan 都有 mapping item、symbol 完全一致、全部 NOT_CONNECTED / SPEC_DEFINED_NOT_CONNECTED 與安全旗標、欄位非空且能對應到 V64 descriptor 與 V63 trade plan 欄位，UI 含必要字串，並**大小寫不敏感**掃描新 contract/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:descriptor-to-real-quote-mapping`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection / production switch 旗標。

架構文件清單新增：Descriptor-to-Real Quote Mapping Readiness Matrix（[docs/descriptor-to-real-quote-mapping-readiness.md](docs/descriptor-to-real-quote-mapping-readiness.md)）。

### V64

Trade Plan Fixture Source Descriptor & Mapping Boundary：

- 把 V63 structured trade plan 用的 fixture price levels（原本散落在 V63 builder 內的 PRICE_LEVELS）抽成獨立 fixture source descriptor contract，並建立 future real quote mapping boundary。本版仍是 fixture-only / deterministic contract / validator / UI structure；**不是 real-data mapping、不是 Supabase connection、不是 production trading readiness。** no Supabase connection、no env read、no DB write、no fetch、no real market data、no API route、no /api/portfolio switch、no buy/sell command、no auto order、not PRODUCTION_READY。不翻任何 manual sign-off / staging connection 旗標。
- 新增 `use-cases/war-room/candidate-price-level-fixture-source-contract.ts`：型別 CandidatePriceLevelFixtureSource、CandidatePriceLevelDescriptor（symbol / name / sourceType = "fixture_mock" / sourceLabel / sourceDescription / fixtureAsOfText / currency = "TWD" / buyZoneLower / buyZoneUpper / stopLossLower / stopLossUpper / targetLower / targetUpper / confidence / operationalUseAllowed = false / realMappingStatus = "NOT_CONNECTED" / futureRealSourceAllowed = false / generatedBuySellCommand = false / autoOrderRequested = false）、CandidatePriceLevelMappingBoundary（realSourceCandidateName / expectedRealFields / mappingNotConnectedReason / requiredEvidenceBeforeConnection / manualSignoffRequired = true / manualSignoffCompleted = false / stagingReadOnlyRequired = true / productionSwitchAllowed = false）、CandidatePriceLevelFixtureSourceBundle、CandidatePriceLevelFixtureSourceValidation 與 constants。
- 新增 `use-cases/war-room/build-candidate-price-level-fixture-source-contract.ts`：pure deterministic builder，擁有 canonical fixture descriptors（A100 / A101 / B200 / C300），逐檔自我驗證（對應 existing candidate symbol、sourceType fixture_mock、realMappingStatus NOT_CONNECTED、operationalUseAllowed / futureRealSourceAllowed / generatedBuySellCommand / autoOrderRequested false、buyZoneLower <= buyZoneUpper、stopLossUpper < buyZoneLower、targetLower > buyZoneUpper），並輸出 mapping boundary；另提供 `buildCandidatePriceLevelDescriptorMap` 供 V63 builder 取得價位。generatedAt 由 input 傳入或固定 `2026-06-23T00:00:00.000Z`，不讀時鐘、不連線、不寫資料。
- 改寫 `use-cases/war-room/build-structured-candidate-trade-plan-contract.ts`：移除手寫 PRICE_LEVELS，改由 descriptor 取得 buy zone / stop / target 價位（descriptor 與 trade plan 價位完全一致），對外 CandidateTradePlan 型別與 UI 使用方式不變。
- 改寫 UI：`components/war-room/daily-candidate-pools.tsx`（每檔結構化 trade plan 加上 fixture source descriptor 標示、realMappingStatus = NOT_CONNECTED、未連真實報價、fixture 區間不可作為正式操作依據）、`components/war-room/war-room-operational-layout.tsx`（import fixture source builder 並傳入候選池），`app/system/safety/page.tsx`（新增 spec-only Mapping Boundary health card：descriptors valid / realMappingStatus / manual sign-off required·completed / production switch·staging read-only）。
- 新增 `scripts/validate-candidate-price-level-fixture-source.ts`：9 gates，import builder 實際呼叫，驗證每個 V63 trade plan 都有 descriptor、每個 descriptor 對應 existing candidate symbol、descriptor 與 trade plan 價位完全一致、mapping boundary 與安全旗標，UI 含 realMappingStatus / NOT_CONNECTED / 未連真實報價 / fixture 區間不可作為正式操作依據、`/system/safety` 含 Mapping Boundary，並**大小寫不敏感**掃描新 contract/builder 禁止 token 與 PRODUCTION_READY、確認未新增 API route。
- 新增 `npm run test:candidate-price-level-fixture-source`。
- 未新增 API route；未連 Supabase；未讀 env；未寫 DB；未 fetch；未接真實行情；未切換 /api/portfolio；未產生買賣指令；未自動下單；未標記 PRODUCTION_READY；未翻 manual sign-off / staging connection 旗標。

架構文件清單新增：Trade Plan Fixture Source Descriptor（[docs/candidate-price-level-fixture-source.md](docs/candidate-price-level-fixture-source.md)）。
