...
- test:limited-live-fetch-timeout-boundary（Timeout Boundary Validator for Limited Live Fetch；expectedDecision OFFLINE_DETERMINISTIC_TIMEOUT_OK、mode OFFLINE_DETERMINISTIC_TIMEOUT_BOUNDARY；**offline deterministic timeout / abort fallback validation** — mock globalThis.fetch + 假化 3000ms abort timer，timeout / abort 後回 safe disabled / non-operational candidate；offline=true、deterministic=true、timeoutBoundary=true、timeoutMs 3000、maxRetries 0、realNetworkUsed=false、fetchMockRestored=true、setTimeoutRestored=true、timeoutAbortSafeFallback=true、receivedAtDeterministic=true、liveFetchPerformed=false、smokeInvoked=false、smokeManualOnly=true、productionDataSwitchAllowed=false、operationalUseAllowed/productionReady false；symbol 3019、channel tse_3019.tw）

> 自 Timeout Boundary Safety Chain Integration 起，guard 共覆蓋 **22** 支 check（V60–V72 + Phase 2 locked implementation + Phase 2b shadow comparison UI shell + Staging Shadow Runtime Scaffold + Limited Live Fetch Dry-run PR Scope + Limited Live Fetch Dry-run Implementation + Golden Snapshot Validator for Limited Live Fetch + Mock Fetch Boundary Validator for Limited Live Fetch + Default No-Fetch Boundary Validator for Limited Live Fetch + Timeout Boundary Validator for Limited Live Fetch）。
>
...
>
> Timeout boundary 是 **offline deterministic timeout / abort fallback validation**：mock globalThis.fetch 並把 provider 的 3000ms abort `setTimeout` 假化為 microtask 觸發（讓 AbortController 真的 abort、不等真 3000ms），驗證 timeout / abort 後回 safe disabled / non-operational candidate、receivedAt deterministic，測試後還原 fetch 與 setTimeout；**不打真網路、不跑 smoke、no production data switch**。
>
