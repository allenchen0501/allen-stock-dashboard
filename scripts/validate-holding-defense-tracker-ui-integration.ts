/**
 * Holding Defense Tracker UI Integration Validator — V29
 *
...
interface HoldingDefenseUiSummary {
  status: CheckStatus;
...
const DOC_REL = "docs/holding-defense-tracker-ui-integration.md";
const COMPONENT_REL = "components/holding-defense-tracker.tsx";
...
function checkRequiredFiles(): CheckResult {
  const required: Array<{ label: string; rel: string }> = [
    { label: "Holding Defense Tracker UI Integration doc (new)", rel: DOC_REL },
    { label: "Holding Defense Tracker component (new)", rel: COMPONENT_REL },
    { label: "Holdings page", rel: PAGE_REL },
    { label: "Holding Defense Tracker API route", rel: ROUTE_REL },
    { label: "Holding Defense Tracker contract", rel: CONTRACT_REL },
...
// Item-card field coverage (presence in component source).
...
function checkComponentSafety(): CheckResult {
  ...
  // The only fetch must target the internal API route.
  if (lower.includes("fetch(")) {
    if (lower.includes("/api/portfolio/holding-defense")) {
      details.push("PASS  Component fetch targets internal /api/portfolio/holding-defense.");
    } else {
      issues.push("FAIL  Component fetch does not target /api/portfolio/holding-defense.");
    }
  } else {
    issues.push("FAIL  Component does not contain a fetch( call.");
  }

  // Must not import the builder or the route module.
  for (const token of ["build-holding-defense-tracker-contract", "portfolio/holding-defense/route"]) {
    if (lower.includes(token)) {
...
function checkPageIntegration(): CheckResult {
  ...
  if (body.includes("HoldingDefenseTracker")) {
    details.push("PASS  HoldingDefenseTracker referenced in holdings page.");
  } else {
    issues.push("FAIL  HoldingDefenseTracker not referenced in holdings page.");
  }
  if (body.includes("components/holding-defense-tracker")) {
    details.push("PASS  holdings page imports components/holding-defense-tracker.");
  } else {
    issues.push("FAIL  holdings page does not import components/holding-defense-tracker.");
  }
  if (body.includes("<HoldingDefenseTracker")) {
    details.push("PASS  <HoldingDefenseTracker /> rendered in holdings page.");
  } else {
    issues.push("FAIL  <HoldingDefenseTracker /> not rendered in holdings page.");
  }
  ...
  if (lower.includes("build-holding-defense-tracker-contract")) {
    issues.push("FAIL  holdings page must not import the builder.");
  } else {
    details.push("PASS  holdings page does not import the builder.");
  }
...
const PKG_TERMS: string[] = [
  '"test:holding-defense-tracker-ui-integration": "node --require ./scripts/register-typescript.cjs ./scripts/validate-holding-defense-tracker-ui-integration.ts"',
];
...
const README_TERMS: string[] = [
  ...
  "npm run test:holding-defense-tracker-ui-integration",
  "/api/portfolio/holding-defense",
  "fixture data 不是即時資料",
  "fixture data 不是投資建議",
  "holdingImpact",
...
// ---------------------------------------------------------------------------
// Gate 7: Safety checks (artifacts / protected layers)
// ---------------------------------------------------------------------------
...
function checkSafety(): CheckResult {
  ...
  const forbiddenArtifacts = [
    ...
    "app/api/portfolio/holding-defense-ui/route.ts",
    "supabase/holding_defense_ui.sql",
    "components/holding-defense.tsx",
  ];
  for (const rel of forbiddenArtifacts) {
    if (fileExists(resolve(rel))) {
      issues.push(`FAIL  Artifact ${rel} must not exist in V29.`);
    } else {
      details.push(`PASS  ${rel} does not exist (correct).`);
    }
  }

  // No holding-defense runtime / quote-polling / scheduler / crawler.
  const forbiddenRuntime = [
    "services/holding-defense/quote-poller.ts",
    "services/holding-defense/scheduler.ts",
    "services/holding-defense/crawler.ts",
    "use-cases/holding-defense/holding-defense-runtime.ts",
  ];
  for (const rel of forbiddenRuntime) {
    if (fileExists(resolve(rel))) {
      issues.push(`FAIL  Runtime ${rel} must not exist in V29.`);
    } else {
      details.push(`PASS  ${rel} does not exist (correct).`);
    }
  }

  // Protected layers must still be present (V27 API route / contract / builder
  // and War Room must not be deleted).
  ...
  // V27 API route must remain unmodified-in-spirit: still calls the builder.
  const routeBody = readFile(resolve(ROUTE_REL));
  if (routeBody == null) {
    issues.push(`FAIL  Cannot read ${ROUTE_REL}.`);
  } else if (routeBody.includes("buildHoldingDefenseTrackerContract")) {
    details.push("PASS  V27 route still calls buildHoldingDefenseTrackerContract.");
  } else {
    issues.push("FAIL  V27 route may have been altered (builder call missing).");
  }
...
const summary: HoldingDefenseUiSummary = {
  status: overallStatus,
  checked_files: [DOC_REL, COMPONENT_REL, PAGE_REL, ROUTE_REL, CONTRACT_REL, README_REL, PKG_REL],
  gates: {
