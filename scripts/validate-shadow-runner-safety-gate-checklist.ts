/**
 * Shadow Runner Safety Gate Checklist Validator — V54
 *
 * Contract / spec-only check. Imports the pure builder + constants and inspects
 * the checklist bundle; it does NOT start a Next.js server, make any HTTP
...
const builderModule = require("../use-cases/deployment/build-shadow-runner-safety-gate-checklist-contract") as typeof import("../use-cases/deployment/build-shadow-runner-safety-gate-checklist-contract");
const { buildShadowRunnerSafetyGateChecklistContract } = builderModule;
...
interface ChecklistSummary {
  ...
  checklist_categories_covered: number;
  issues: string[];
  warnings: string[];
...
const DOC_REL = "docs/shadow-runner-safety-gate-checklist.md";
const CONTRACT_REL = "use-cases/deployment/shadow-runner-safety-gate-checklist-contract.ts";
const BUILDER_REL = "use-cases/deployment/build-shadow-runner-safety-gate-checklist-contract.ts";
...
function checkRequiredFiles(): CheckResult {
  const required: Array<{ label: string; rel: string }> = [
    { label: "Checklist doc (new)", rel: DOC_REL },
    { label: "Checklist contract (new)", rel: CONTRACT_REL },
    { label: "Checklist builder (new)", rel: BUILDER_REL },
...
const REQUIRED_DOC_PHRASES: string[] = [
  ...
  "V55 Staging Read-only Connection Review Gate",
];
...
const CONTRACT_TERMS: string[] = [
  "ShadowRunnerSafetyGateDeploymentTarget",
  ...
  "SHADOW_RUNNER_SAFETY_GATE_CHECKLIST_CONTRACT_VERSION",
  "SHADOW_RUNNER_SAFETY_GATE_CHECKLIST_CATEGORIES",
  "SHADOW_RUNNER_SAFETY_GATE_CHECKLIST_ALLOWED_DECISIONS",
  "SHADOW_RUNNER_SAFETY_GATE_CHECKLIST_SAFETY_LABELS",
  "SHADOW_RUNNER_SAFETY_GATE_CHECKLIST_DISALLOWED_TERMS",
  "READY_FOR_REVIEW",
  "NO_GO",
...
const EXPECTED_CATEGORIES = [
  "DEPLOYMENT", "ROUTE", "MONITORING_UI", "SUPABASE_SAFETY", "RLS", "SCHEMA_MAPPING", "ADAPTER",
  "SHADOW_COMPARISON", "SHADOW_RUNNER", "API_CONTRACT", "PRODUCTION_EVIDENCE", "DATA_SOURCE",
  "PORTFOLIO_SWITCH", "TRADING_SAFETY", "MANUAL_SIGNOFF",
];

function checkPayload(): { result: CheckResult; itemCount: number; categoriesCovered: number } {
  const details: string[] = [];
  const issues: string[] = [];

  const p = buildShadowRunnerSafetyGateChecklistContract({ generatedAt: FIXED_TS });
  ...
  expectEq("contractVersion", p.contractVersion, "V54");
  expectEq("checklistName", p.checklistName, "Shadow Runner Safety Gate Checklist");
  expectEq("deploymentTarget", p.deploymentTarget, "staging");
  expectEq("generatedAt", p.generatedAt, FIXED_TS);

  if (p.decision === "READY_FOR_REVIEW" || p.decision === "NO_GO") details.push(`PASS  decision = ${p.decision} (allowed).`);
  else issues.push(`FAIL  decision = ${p.decision}, expected READY_FOR_REVIEW or NO_GO.`);
  ...
  // Frozen safety flags.
  const trueFlags = ["safetyGateChecklistDefined", "manualSignoffRequired", "killSwitchDefaultEnabled", "productionRouteSmokePassed", "monitoringUiProductionEvidencePassed"] as const;
  const falseFlags = [
    "manualSignoffCompleted", "stagingConnectionAllowed", "stagingSupabaseConnected", "stagingReadPerformed",
    "stagingWritePerformed", "productionSupabaseConnected", "productionWritePerformed", "envReadPerformed",
    "databaseWritePerformed", "requestPerformed", "apiRouteCreated", "uiCreated", "sqlMigrationCreated",
    "runtimeCreated", "shadowRunnerRuntimeCreated", "shadowRunnerExecuted", "shadowComparisonPerformed",
    "shadowResultPersisted", "portfolioApiSwitched", "portfolioSourceModeChanged", "realMarketDataEnabled",
    "buySellCommandGenerated", "autoOrderRequested", "fixtureCanBeOverriddenByStaging",
  ...
  if (items.length >= 18) details.push(`PASS  checklistItems.length = ${items.length} (>= 18).`);
  else issues.push(`FAIL  checklistItems.length = ${items.length}, expected >= 18.`);
  ...
  if (policyOk) details.push("PASS  All checklist items satisfy policy expectations.");

  for (const label of ["no buy/sell command", "no Supabase connection", "Shadow Runner Safety Gate Checklist"]) {
...
const PKG_TERMS: string[] = [
  '"test:shadow-runner-safety-gate-checklist": "node --require ./scripts/register-typescript.cjs ./scripts/validate-shadow-runner-safety-gate-checklist.ts"',
];

const README_TERMS: string[] = [
  "V54",
  "Shadow Runner Safety Gate Checklist",
  "docs/shadow-runner-safety-gate-checklist.md",
  "use-cases/deployment/shadow-runner-safety-gate-checklist-contract.ts",
  "use-cases/deployment/build-shadow-runner-safety-gate-checklist-contract.ts",
  "npm run test:shadow-runner-safety-gate-checklist",
  "manualSignoffRequired = true",
  "manualSignoffCompleted = false",
  "stagingConnectionAllowed = false",
  "V55 Staging Read-only Connection Review Gate",
...
function checkSafety(): CheckResult {
  ...
  const forbiddenArtifacts = [
    "app/api/portfolio/shadow-runner-safety-gate-checklist/route.ts",
    "components/shadow-runner-safety-gate-checklist.tsx",
    "supabase/shadow_runner_safety_gate_checklist.sql",
  ];
  for (const rel of forbiddenArtifacts) {
    if (fileExists(resolve(rel))) issues.push(`FAIL  Artifact ${rel} must not exist in V54.`);
...
const { result: payloadCheck, itemCount, categoriesCovered } = checkPayload();
...
const payloadForFlags = buildShadowRunnerSafetyGateChecklistContract({ generatedAt: FIXED_TS });
...
const summary: ChecklistSummary = {
  ...
  checklist_categories_covered: categoriesCovered,
  issues: allIssues,
  warnings: allWarnings,
  manual_signoff_required: payloadForFlags.manualSignoffRequired,
  manual_signoff_completed: payloadForFlags.manualSignoffCompleted,
  staging_connection_allowed: payloadForFlags.stagingConnectionAllowed,
  production_write_performed: false,
  request_performed: false,
  supabase_connected: false,
  env_read_performed: false,
