/**
 * Staging Read-only Dry-run Execution Evidence Spec Validator — V59
 *
...
const builderModule = require("../use-cases/deployment/build-staging-readonly-dry-run-execution-evidence-spec-contract") as typeof import("../use-cases/deployment/build-staging-readonly-dry-run-execution-evidence-spec-contract");
const { buildStagingReadonlyDryRunExecutionEvidenceSpecContract } = builderModule;
...
interface EvidenceSummary {
  ...
  actual_dry_run_evidence_provided: boolean;
...
const DOC_REL = "docs/staging-readonly-dry-run-execution-evidence-spec.md";
const CONTRACT_REL = "use-cases/deployment/staging-readonly-dry-run-execution-evidence-spec-contract.ts";
const BUILDER_REL = "use-cases/deployment/build-staging-readonly-dry-run-execution-evidence-spec-contract.ts";
...
function checkRequiredFiles(): CheckResult {
  const required: Array<{ label: string; rel: string }> = [
    { label: "Execution evidence spec doc (new)", rel: DOC_REL },
    { label: "Execution evidence spec contract (new)", rel: CONTRACT_REL },
    { label: "Execution evidence spec builder (new)", rel: BUILDER_REL },
...
const REQUIRED_DOC_PHRASES: string[] = [
  ...
  "kill switch",
  "V60 Manual Sign-off Evidence Instance",
  "V60 Staging Read-only Dry-run Execution Evidence Instance",
];
...
const CONTRACT_TERMS: string[] = [
  "StagingReadonlyDryRunExecutionEvidenceDeploymentTarget",
  ...
  "STAGING_READONLY_DRY_RUN_EXECUTION_EVIDENCE_SPEC_CONTRACT_VERSION",
  "STAGING_READONLY_DRY_RUN_EXECUTION_EVIDENCE_SPEC_CATEGORIES",
  "STAGING_READONLY_DRY_RUN_EXECUTION_EVIDENCE_SPEC_ALLOWED_DECISIONS",
  "STAGING_READONLY_DRY_RUN_EXECUTION_EVIDENCE_SPEC_SAFETY_LABELS",
  "STAGING_READONLY_DRY_RUN_EXECUTION_EVIDENCE_SPEC_DISALLOWED_TERMS",
  "NO_GO",
  "READY_FOR_REVIEW",
...
const EXPECTED_CATEGORIES = [
  "EXECUTION_IDENTITY", "MANUAL_SIGNOFF", "READ_ONLY_PROBE", "NO_WRITE_PROOF", "RLS_POLICY", "ROLE_ACCESS",
  "SHADOW_COMPARISON", "MISMATCH_CLASSIFICATION", "STALE_EMPTY_ERROR_FALLBACK", "KILL_SWITCH", "ROLLBACK",
  "PORTFOLIO_SOURCE_MODE", "API_ROUTE_SAFETY", "DATA_SOURCE_SAFETY", "TRADING_SAFETY", "FINAL_REVIEW",
];
...
function checkPayload(): { result: CheckResult; itemCount: number; categoriesCovered: number; decision: string } {
  ...
  const p = buildStagingReadonlyDryRunExecutionEvidenceSpecContract({ generatedAt: FIXED_TS });
  ...
  expectEq("contractVersion", p.contractVersion, "V59");
  expectEq("specName", p.specName, "Staging Read-only Dry-run Execution Evidence Spec");
  ...
  const trueFlags = [
    "dryRunExecutionEvidenceSpecDefined", "manualSignoffRequired", "killSwitchDefaultEnabled", "dashboardReadonlyRoleRequired",
    "readOnlySelectOnlyRequired", "writeOperationsBlocked", "shadowOnlyRequired", "portfolioApiMustRemainHardcoded",
  ] as const;
  const falseFlags = [
    "actualDryRunExecuted", "actualDryRunEvidenceProvided", "actualConnectionImplemented", "actualConnectionAttempted",
  ...
  const items = p.evidenceItems;
  if (items.length >= 20) details.push(`PASS  evidenceItems.length = ${items.length} (>= 20).`);
  else issues.push(`FAIL  evidenceItems.length = ${items.length}, expected >= 20.`);
  ...
  for (const i of items) {
    ...
    for (const b of ["requiredAfterDryRunExecution", "blocksEvidenceAcceptance", "blocksProductionReadiness", "manualReviewRequired"] as const) {
      if (typeof i[b] !== "boolean") { policyOk = false; issues.push(`FAIL  ${id} ${b} not boolean.`); }
    }
    if (i.requiredAfterDryRunExecution && i.status !== "PASS" && !i.blocksEvidenceAcceptance) {
      policyOk = false; issues.push(`FAIL  ${id} requiredAfterDryRunExecution non-PASS must blocksEvidenceAcceptance=true.`);
    }
  }
  if (policyOk) details.push("PASS  All evidence items satisfy policy expectations.");

  if (p.decision !== "NO_GO") issues.push(`FAIL  decision must be NO_GO while dry-run not executed (got ${p.decision}).`);
  else details.push("PASS  decision is NO_GO (dry-run not executed).");

  for (const label of ["no buy/sell command", "no Supabase connection", "Staging Read-only Dry-run Execution Evidence Spec"]) {
...
const PKG_TERMS: string[] = [
  '"test:staging-readonly-dry-run-execution-evidence-spec": "node --require ./scripts/register-typescript.cjs ./scripts/validate-staging-readonly-dry-run-execution-evidence-spec.ts"',
];

const README_TERMS: string[] = [
  "V59",
  "Staging Read-only Dry-run Execution Evidence Spec",
  "docs/staging-readonly-dry-run-execution-evidence-spec.md",
  "use-cases/deployment/staging-readonly-dry-run-execution-evidence-spec-contract.ts",
  "use-cases/deployment/build-staging-readonly-dry-run-execution-evidence-spec-contract.ts",
  "npm run test:staging-readonly-dry-run-execution-evidence-spec",
  "actualDryRunExecuted = false",
  "actualDryRunEvidenceProvided = false",
  "stagingDryRunExecutionAllowed = false",
  "V60 Manual Sign-off Evidence Instance",
...
function checkSafety(): CheckResult {
  ...
  const forbiddenArtifacts = [
    "app/api/portfolio/staging-readonly-dry-run-execution-evidence/route.ts",
    "components/staging-readonly-dry-run-execution-evidence.tsx",
    "supabase/staging_readonly_dry_run_execution_evidence.sql",
  ];
  for (const rel of forbiddenArtifacts) {
    if (fileExists(resolve(rel))) issues.push(`FAIL  Artifact ${rel} must not exist in V59.`);
...
const summary: EvidenceSummary = {
  ...
  actual_dry_run_evidence_provided: p.actualDryRunEvidenceProvided,
