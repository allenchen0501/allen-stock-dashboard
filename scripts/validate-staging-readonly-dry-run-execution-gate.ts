/**
 * Staging Read-only Dry-run Execution Gate Validator — V58
 *
...
const builderModule = require("../use-cases/deployment/build-staging-readonly-dry-run-execution-gate-contract") as typeof import("../use-cases/deployment/build-staging-readonly-dry-run-execution-gate-contract");
const { buildStagingReadonlyDryRunExecutionGateContract } = builderModule;
...
interface GateSummary {
  ...
  gate_categories_covered: number;
  decision: string;
  ...
  actual_dry_run_executed: boolean;
  actual_connection_attempted: boolean;
  issues: string[];
  warnings: string[];
...
const DOC_REL = "docs/staging-readonly-dry-run-execution-gate.md";
const CONTRACT_REL = "use-cases/deployment/staging-readonly-dry-run-execution-gate-contract.ts";
const BUILDER_REL = "use-cases/deployment/build-staging-readonly-dry-run-execution-gate-contract.ts";
...
function checkRequiredFiles(): CheckResult {
  const required: Array<{ label: string; rel: string }> = [
    { label: "Execution gate doc (new)", rel: DOC_REL },
    { label: "Execution gate contract (new)", rel: CONTRACT_REL },
    { label: "Execution gate builder (new)", rel: BUILDER_REL },
...
const REQUIRED_DOC_PHRASES: string[] = [
  ...
  "kill switch",
  "V59 Manual Sign-off Evidence Instance",
  "V59 Staging Read-only Dry-run Execution Evidence",
];
...
const CONTRACT_TERMS: string[] = [
  "StagingReadonlyDryRunExecutionDeploymentTarget",
  ...
  "STAGING_READONLY_DRY_RUN_EXECUTION_GATE_CONTRACT_VERSION",
  "STAGING_READONLY_DRY_RUN_EXECUTION_GATE_CATEGORIES",
  "STAGING_READONLY_DRY_RUN_EXECUTION_GATE_ALLOWED_DECISIONS",
  "STAGING_READONLY_DRY_RUN_EXECUTION_GATE_SAFETY_LABELS",
  "STAGING_READONLY_DRY_RUN_EXECUTION_GATE_DISALLOWED_TERMS",
  "NO_GO",
  "READY_FOR_REVIEW",
...
const EXPECTED_CATEGORIES = [
  "MANUAL_SIGNOFF", "EVIDENCE_REQUIREMENTS", "SUPABASE_PROJECT", "ENVIRONMENT_VARIABLES", "RLS_POLICY", "ROLE_ACCESS",
  "READ_ONLY_OPERATION", "SHADOW_ONLY", "KILL_SWITCH", "ROLLBACK", "PORTFOLIO_SOURCE_MODE", "API_ROUTE_SAFETY",
...
function checkPayload(): { result: CheckResult; itemCount: number; categoriesCovered: number; decision: string } {
  ...
  const p = buildStagingReadonlyDryRunExecutionGateContract({ generatedAt: FIXED_TS });
  ...
  expectEq("contractVersion", p.contractVersion, "V58");
  expectEq("gateName", p.gateName, "Staging Read-only Dry-run Execution Gate");
  ...
  const trueFlags = [
    "dryRunExecutionGateDefined", "v57DryRunPlanExists", "v56ManualSignoffSpecExists", "v55ConnectionReviewGateExists",
    "manualSignoffRequired", "killSwitchDefaultEnabled", "dashboardReadonlyRoleRequired", "readOnlySelectOnlyRequired",
    "writeOperationsBlocked", "shadowOnlyRequired", "portfolioApiMustRemainHardcoded",
  ] as const;
  const falseFlags = [
    "actualDryRunExecuted", "actualConnectionImplemented", "actualConnectionAttempted", "manualSignoffCompleted",
    "manualSignoffEvidenceProvided", "manualSignerIdentityVerified", "manualSignerHasAuthority", "stagingConnectionAllowed",
    "stagingConnectionReviewAllowed", "stagingDryRunExecutionAllowed", "productionReadinessAllowed", "stagingSupabaseConnected",
    "stagingReadPerformed", "stagingWritePerformed", "productionSupabaseConnected", "productionWritePerformed",
    "envReadPerformed", "databaseWritePerformed", "requestPerformed", "apiRouteCreated", "uiCreated", "sqlMigrationCreated",
    "runtimeCreated", "shadowRunnerRuntimeCreated", "shadowRunnerExecuted", "shadowComparisonPerformed", "shadowResultPersisted",
    "portfolioApiSwitched", "portfolioSourceModeChanged", "realMarketDataEnabled", "buySellCommandGenerated", "autoOrderRequested",
    "serviceRoleAllowedInAppRuntime", "anonRoleAllowed", "fixtureCanBeOverriddenByStaging", "hardcodedCanBeOverriddenByStaging",
    "mismatchCanPromoteStaging", "dryRunCanPromoteStaging", "emptyResultCanOverrideHardcoded", "staleResultCanOverrideHardcoded",
  ...
  const items = p.gateItems;
  if (items.length >= 20) details.push(`PASS  gateItems.length = ${items.length} (>= 20).`);
  else issues.push(`FAIL  gateItems.length = ${items.length}, expected >= 20.`);
  ...
  for (const i of items) {
    ...
    if (!i.failureAction || i.failureAction.length === 0) { policyOk = false; issues.push(`FAIL  ${id} failureAction empty.`); }
  ...
  if (policyOk) details.push("PASS  All gate items satisfy policy expectations.");
  ...
  for (const label of ["no buy/sell command", "no Supabase connection", "Staging Read-only Dry-run Execution Gate"]) {
...
const PKG_TERMS: string[] = [
  '"test:staging-readonly-dry-run-execution-gate": "node --require ./scripts/register-typescript.cjs ./scripts/validate-staging-readonly-dry-run-execution-gate.ts"',
];

const README_TERMS: string[] = [
  "V58",
  "Staging Read-only Dry-run Execution Gate",
  "docs/staging-readonly-dry-run-execution-gate.md",
  "use-cases/deployment/staging-readonly-dry-run-execution-gate-contract.ts",
  "use-cases/deployment/build-staging-readonly-dry-run-execution-gate-contract.ts",
  "npm run test:staging-readonly-dry-run-execution-gate",
  "manualSignoffCompleted = false",
  "stagingDryRunExecutionAllowed = false",
  "actualDryRunExecuted = false",
  "V59 Manual Sign-off Evidence Instance",
...
function checkSafety(): CheckResult {
  ...
  const forbiddenArtifacts = [
    "app/api/portfolio/staging-readonly-dry-run-execution-gate/route.ts",
    "components/staging-readonly-dry-run-execution-gate.tsx",
    "supabase/staging_readonly_dry_run_execution_gate.sql",
  ];
  for (const rel of forbiddenArtifacts) {
    if (fileExists(resolve(rel))) issues.push(`FAIL  Artifact ${rel} must not exist in V58.`);
...
const summary: GateSummary = {
  ...
  gate_item_count: itemCount,
  gate_categories_covered: categoriesCovered,
  decision,
  ...
  actual_dry_run_executed: p.actualDryRunExecuted,
  actual_connection_attempted: p.actualConnectionAttempted,
  issues: allIssues,
